...
Two callers share this module: the ``pandda_fanout`` *task*, which is the
interface's way and leaves a job recording what it created, and the
``pandda_fanout`` management command, for scripting.
"""
import json
...
def create_receipt(project: models.Project, xtal: str, label: str, tree: Path,
    ...
    for path, value in values:
        ...
        if not getattr(result, "success", False):
            raise RuntimeError(f"could not set {path}={value!r} on job {job.number}: "
