...
class phaser_MR_PAK_report(Report):
    ...
    def addBestSolution(self, parent=None):
        if parent is None: parent = self
        # xpath0() took the first hit and is gone with the Qt-era report parser;
        # find() is its equivalent, but it can return None where xpath0() raised.
        solutionNode = self.xmlnode.find('PhaserCurrentBestSolution/Solution')
