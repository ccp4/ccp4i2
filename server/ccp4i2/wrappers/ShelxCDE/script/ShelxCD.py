...
class ShelxCD(CPluginScript):
    ...
    def runShelxc( self ):
        ...
        try:
            ...
            print('#ShelxCD runShelxc: SFAC unset or not relevant')
        ...
        try:
            ...
            print('#ShelxCD runShelxc: FIND unset or not relevant')
        
        try:
            if self.container.controlParameters.NTRY.isSet():
                inputText += 'NTRY %d\n'%int(self.container.controlParameters.NTRY)
        except:
            print('#ShelxCD runShelxc: NTRY unset or not relevant')
    ...
    def scrapeShelxcLog(self, xmlroot, logFile):
        ...
        with open(logFile,'r') as insFile:
            ...
            for line in insLines:
                ...
                elif len(tokens) > 0 and tokens[0] in ShelxCD.tagsAndKeys and analysisNode is not None:
                    for iBin in range (1, len(tokens)):
                        binNode = analysisNode[iBin-1]
                        self.subElementOfTypeWithText(binNode,ShelxCD.tagsAndKeys[tokens[0]], tokens[iBin])
...
'''
        ...
        for coord in coords:
            ...
            element = [ element for element in ShelxCD.PeriodicTable if element['Number'] == coord['AtomicNr']]
    ...
    def resToPDB(self, inputResPath, outputPDBPath):
        ...
        with open(outputFracPath,'w') as outputHA:
            ...
            iCoord = 1
            for coord in coords:
                element = [ element for element in ShelxCD.PeriodicTable if element['Symbol'].upper() == coord['Element']][0]
