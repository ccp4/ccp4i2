...
class StagedUpload(Model):
    """
    Tracks staged file uploads for large files that bypass HTTP body limits.

    Flow:
    1. Client requests upload URL → creates StagedUpload with status=pending
    2. Client uploads directly to blob storage using SAS URL
    3. Client calls complete endpoint → status=processing, triggers import
    4. After processing → status=completed or status=failed
    """
    ...
    # Blob storage path (relative to container)
    blob_path = CharField(max_length=512)

    # SAS URL expiry time
    sas_expiry = DateTimeField()

    # For unmerged_data uploads, link to the target job
    target_job = ForeignKey(Job, SET_NULL, blank=True, null=True, related_name="staged_uploads")
    ...
    # User who requested the upload (stored as string for simplicity)
    requested_by = CharField(max_length=255, blank=True)

    def __str__(self):
        return f"{self.upload_type}: {self.original_filename} ({self.status})"
