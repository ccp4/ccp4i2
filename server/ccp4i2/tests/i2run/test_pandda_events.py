...
def test_complete_receipt_gleans_every_nested_file(tmp_path):
    ...
    with i2run(["pandda_events", "--PANDDA_OUT_DIR", str(tree), "--DTAG", "xtal-0004"]) as job:
        ...
        # PanDDA wrote its maps as P1; the copies carry the crystal's space
        # group from the apo model, so coot treats them as the X-ray maps they
        # are; the tree is untouched.
        import gemmi
        assert gemmi.read_ccp4_map(str(job / "ZMAP.map")).grid.spacegroup.number == 20
        assert gemmi.read_ccp4_map(str(job / "event_1_map.map")).grid.spacegroup.number == 20
