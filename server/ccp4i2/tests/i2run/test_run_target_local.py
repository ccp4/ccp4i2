...
def test_a_dispatched_job_runs_to_completion_on_the_local_target(tmp_path):
    env = _env(tmp_path)
...
manifest.write_text(json.dumps({{
    ...
    "provenance": {{}},
    "datasets": [
        {{"xtal": "xtal-0000", "label": "rt_m1", "project_uuid": str(members[0].uuid), "files": {{}}}},
        {{"xtal": "xtal-0001", "label": "rt_m2", "project_uuid": str(members[1].uuid), "files": {{}}}},
    ]}}))
print(tree); print(manifest)
"""
    r = subprocess.run([sys.executable, "-c", setup, str(tmp_path)], cwd=SERVER_DIR, env=env,
...
call_command("i2run")
"""
    r = subprocess.run([sys.executable, "-c", i2run, "pandda_fanout", "--project_name", "rt_parent",
    ...
    assert rows == [("rt_m1", 6), ("rt_m2", 6)], rows


def test_an_unknown_job_target_from_the_environment_is_reported_not_worked_around(tmp_path):
...
from ccp4i2.db import models
from {SYNTHETIC} import event_record, make_tree
d = Path(settings.CCP4I2_PROJECTS_DIR) / "rt_x"; (d / "CCP4_JOBS").mkdir(parents=True, exist_ok=True)
m = models.Project.objects.create(name="rt_x", directory=str(d))
...
manifest.write_text(json.dumps({{"manifest_version": 1, "created": "now", "datasets_dir": "datasets",
    ...
    "datasets": [{{"xtal": "xtal-0000", "label": "rt_x", "project_uuid": str(m.uuid), "files": {{}}}}]}}))
print(tree); print(manifest)
"""
    r = subprocess.run([sys.executable, "-c", setup, str(tmp_path)], cwd=SERVER_DIR, env=env,
                       capture_output=True, text=True, timeout=300)
...
call_command("i2run")
"""
    r = subprocess.run([sys.executable, "-c", i2run, "pandda_fanout", "--project_name", "rt_xparent",
                        "--MANIFEST", manifest, "--PANDDA_OUT_DIR", tree, "--RUN_RECEIPTS", "True"],
                       cwd=SERVER_DIR, env=env, capture_output=True, text=True, timeout=600)
    db = sqlite3.connect(tmp_path / "db.sqlite")
