...
| 0.6.0 | ✅ | ✅ | Published 2026-09-26 (tag `ccp4i2-api-v0.6.0` on #631, the lockfile catch-up; the bump itself was #629). `VersionInfo` gains `run_targets` / `job_target` (the run targets a deployment registered and what each can do; ccp4i2 #629) and types the `import_staging` field the server already sent. Minor: additive fields, existing surface unchanged. TypeScript-only change; the Python half is unchanged but ships in lockstep. |
| 0.4.0 | ✅ | — | File grants: scoped, expiring read capabilities for the requests a browser issues for itself (a report page's own images, stylesheets and relative fetches), which cannot carry a bearer token. Minor per the policy below — new capability, existing surface unchanged. npm publish failed: `npm install -g npm@latest` resolved npm 12, which requires Node ≥22.22, on a Node 20 runner. Pinned to npm 11.x afterwards. The TypeScript half is byte-identical to 0.3.5, so nothing is missing from npm; the next release with TypeScript changes re-aligns the versions. (0.5.0 did, on 2026-09-21.) |
