    ...
    def listPluginErrorReport(self, jobId):
        try:
            ...
            if len(report) > 0:
                ...
                if hasattr(self, 'currentJobNode'):
                    ET.SubElement(self.currentJobNode,'Warnings').text = report.report(ifStack=True, mode=0, minSeverity=Severity.WARNING)
                    if report.maxSeverity() >= Severity.ERROR:
                        ET.SubElement(self.currentJobNode,'Errors').text = report.report(ifStack=True, mode=0, minSeverity=Severity.ERROR)
