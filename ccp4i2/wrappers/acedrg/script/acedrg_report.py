...
class acedrg_report(Report):
    ...
    def analyseGeometry(self,parent=None):
        ...
        for categoryName, label in categoryNameMap.items():
            examples = self.xmlnode.findall('.//Acedrg/Geometry/'+categoryName)
            if len(examples) > 0:
                folder = parent.addFold(label=label)
    ...
    def analyseEnergies(self, parent=None):
        if parent is None: parent=self
        energyFold = parent.addFold(label='Energy vs. rank')
        graph = energyFold.addFlotGraph(title='Energy Graph', xmlnode=self.xmlnode, select='Conformer')
        graph.addData(title='Rank',select='Rank')
        graph.addData(title='Energy',select='Energy')
        p = graph.addPlotObject()
        p.append('label','Energy vs rank of conformers')
        p.append('plottype','xy')
        p.append('xintegral','true')
        p.append('plotline',xcol=1,ycol=2)
