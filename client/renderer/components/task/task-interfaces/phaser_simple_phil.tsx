...
/**
 * The one-model Phaser MR pipeline over Phaser's own PHIL: a search model, copies, plus a
 * Free-R set, a reference structure and the follow-on steps. The typed inputs are laid out by hand;
...
const TaskInterface: React.FC<CCP4i2TaskInterfaceProps> = (props) => {
  ...
  const { value: idRms } = useTaskItem("ID_RMS");
  const { value: inputFixed } = useTaskItem("INPUT_FIXED");
      ...
      <CCP4i2Tabs>
        <CCP4i2Tab key="inputData" label="Input data">
          ...
          >
            ...
            <CCP4i2TaskElement itemName="NCOPIES" {...props} />
            <CCP4i2TaskElement itemName="ID_RMS" {...props} />
            <CCP4i2TaskElement
              itemName="SEARCHSEQUENCEIDENTITY"
              {...props}
              visibility={() => idRms !== "RMS"}
            />
            <CCP4i2TaskElement
              itemName="SEARCHRMS"
              {...props}
              visibility={() => idRms === "RMS"}
            />
            <CCP4i2TaskElement itemName="INPUT_FIXED" {...props} />
            <CCP4i2TaskElement
              itemName="XYZIN_FIXED"
              {...props}
              visibility={() => Boolean(inputFixed)}
            />
          </CCP4i2ContainerElement>
        ...
        <CCP4i2Tab key="afterwards" label="After Phaser">
          <CCP4i2ContainerElement
            {...props}
            itemName=""
            qualifiers={{ guiLabel: "Reference structure" }}
            containerHint="FolderLevel"
          >
