  ...
  return z
    ...
    .superRefine((s, ctx) => {
      ...
      (s.superpose ?? []).forEach((sp, i) => {
        ...
        if (sp.method === "matrix") {
          // A matrix superposition carries its reference (if any) under
          // `fitted`, and a file may legitimately be fitted onto itself
          // there -- the matrix is the transform, not a comparison.
          if (sp.fitted)
