...
#FIXME (SJM 3/10/2019) - I'm hacking at the moment by still creating the CTOolbar on Mac and hiding it. This is because the toolbar and signals are intimately entwined at the moment. I have to disentagle.
    ...
    def openDictionary(self, state):
        if self._dictionaryWidget is None or (not isAlive(self._dictionaryWidget)):
            from qtgui import CCP4ModelWidgets
    ...
    def handleNextTask(self,taskName,patchParamsFile):
        ...
        if taskName == 'clone_rerun':
            self.cloneTask()
                ...
                suggested = tree.findall("Verdict/suggestedParameters")
    ...
    def showBibliography(self, jobId=None, taskNameList=[]):
        ...
        from qtgui import CCP4BibliographyViewer
        if jobId is None:
            ...
            if 'exe' in args and args['exe'] == 'loggraph':
                ...
                if args['action'] == "view_text":
                    if args["filetypeclass"] == "AsuDataFile":
                        ...
                        seqs = tree.findall(".//CAsuContentSeq")
                        html = """<body>"""
                        for seq in seqs:
                            desc = seq.findall("description")[0].text
                            nCopies = seq.findall("nCopies")[0].text
                            polymerType = seq.findall("polymerType")[0].text
                            sequence = seq.findall("sequence")[0].text
                            if nCopies == 1:
