...
class ProtocolViewSet(ReversionMixin, viewsets.ModelViewSet):
    ...
    filterset_fields = ['import_type', 'analysis_method']
...
class AssayViewSet(ReversionMixin, viewsets.ModelViewSet):
    ...
    def import_table_of_values(self, request, pk=None):
        ...
        if assay.protocol.import_type != 'table_of_values':
            return Response({
                'status': 'error',
                'error': f"Protocol import type is '{assay.protocol.import_type}', not 'table_of_values'"
            }, status=status.HTTP_400_BAD_REQUEST)
...
def available_validation_flags(request):
    """
    Return available fitting flags with descriptions for UI configuration.

    These flags are generated by curve fitting algorithms and can be configured
    per-protocol to determine which flags should cause automatic invalidation.
    """
    return Response({
        'flags': AVAILABLE_FLAGS,
        'default_invalidating': DEFAULT_INVALIDATING_FLAGS,
    })
