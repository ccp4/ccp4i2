...
| 17 | **Extensible scaffold catalog** — runtime DB-backed extension to the curated Python seed. New `ScaffoldExtension` model with project-scoped (`target` FK) and shared (`target = null`) entries; resolver consults `project → shared → seed` in priority order. Inline *"Define this fragment"* affordance on `ScaffoldMiss` lets chemists grow the shared catalog without redeploying. SMARTS validated server-side via RDKit before persisting. | Shipped | §19.3 refinement |
| 18 | **Same-name User+Supplier union chip** — when a typed phrase matches a User AND a same-named unlinked Supplier (e.g. *"Hannah Stewart"* who registered three compounds and supplies two more via her personal-Supplier row), surface ONE chip instead of two role-chips. Picking it pins both ids; executor's filter ORs all three paths (`registered_by | supplier__user | supplier`). Cross-tier merger: a User exact-match via first name still merges with a Supplier substring-match via full name. | Shipped | §6 (registrant resolution refinement) |
...
| 23 | **Scatter view + selection-membership colouring** — promote scatter from a row-pencil dialog to a top-level format mode (`format=scatter`), driven by NLP `view_format: "scatter"` and via the format toggle on the aggregation page. Saved selections become a categorical legend: typing *"...coloured by my CDK4 hits"* maps each named selection (`categorisation_selection_phrases`, fuzzy-resolved per-user) to a colour group, points are partitioned across one dataset per selection plus a grey "Other". URL contract: `?colour_by=<uuid>,<uuid>`. Inline `ScatterCategorisationStrip` chip strip lets the chemist toggle colour groups directly. | Superseded by 24 | §10 follow-up |
| 24 | **Chemotype categorisation + on-the-fly SMARTS** — supersede slice 23's selection-membership colouring with **chemotype membership** as the primary axis. Scatter chip strip lists scaffolds from the substructures catalog (slice 13's curated seed + slice 17's `ScaffoldExtension` rows); client-side RDKit substructure matching computes which compounds carry each scaffold; one dataset per scaffold-membership group. Adds an inline SMARTS input ("+ SMARTS") for ad-hoc patterns the catalog doesn't cover, encoded as `colour_by=smarts:<smarts>`. URL contract: `?colour_by=<scaffold-name>,<scaffold-name>,smarts:<pattern>`. NLP layer: `colour_by_scaffold_phrases` on `CompoundSelector` resolves to canonical names via the catalog. Slice 23's saved-selection plumbing is parked for re-use as a future *shape_by* axis (selection membership as marker shape, giving quasi-3D info on the scatter). | Shipped | §10 follow-up, primary categorisation mechanism |
| 25 | **ScaffoldExtension visibility + colour-by bug-fix + cards-overflow fallback** — three pragmatic fixes ahead of the DDU→demo handover. (a) NLP colour-by phrase resolution now uses `resolve_scaffold` against the combined catalog (project ScaffoldExtension → shared ScaffoldExtension → seed) instead of the seed-only `lookup_scaffold_by_typed`; *"colour by Pyrrolopyrimidine"* now finds extensions a colleague added. (b) New `/registry/scaffold-extensions` management page lists every catalog entry with raw SMARTS as stored, **canonical aromatic SMARTS** RDKit produces from it, structure thumbnail, and a creator-scoped delete affordance — exposes the chemist's library and reveals when a Kekulé paste was normalised. (c) Cards / pivot / bullets formats fall back to compact when the result count exceeds 200 (the only virtualised view), preventing DOM overload from queries like *"all CDK4 compounds"*. (d) `scaffold-matching.ts` now does a SMILES-first round trip in `buildQueryMol`, so Kekulé-style SMILES pasted into "+ SMARTS" produce aromatic-aware queries — the original symptom that lit this slice. | Shipped | §10 follow-up |
...
**Slice 17 — Extensible scaffold catalog (DB extension to the Python seed)** (310 tests cumulative; +22 for the slice)

The Python seed (`apps/compounds/nlp/substructures.py`) is curated and version-controlled — adding a new fragment requires editing the module, committing, deploying. Slice 17 layers a DB-backed extension catalog on top so chemists can grow the catalog at runtime, scoped either to a specific project or shared across the instance.

- **`ScaffoldExtension` model** in [registry/models.py](apps/compounds/registry/models.py). Fields: `name`, `smarts`, `aliases` (JSON list), `target` FK (null = shared), `notes` (human gloss), and audit columns (`created_by`, `created_at`, `source_prompt`, `llm_generated`). `unique_together = (name, target)` — the same name can legitimately exist twice across scopes (*"Series 1"* in ARd is a different fragment from *"Series 1"* in CDK4, and that's a feature). Migration: `0024_scaffoldextension.py`.
- **Resolver plumbing** in [resolver.py](apps/compounds/nlp/resolver.py). New `_combined_scaffold_pool(target)` returns a priority-ordered list of `(scaffold, source, pinning_id)` tuples — project extensions first, then shared extensions, then the Python seed. `resolve_scaffold(as_typed, target=None, ...)` runs the existing three-tier match (exact → substring → fuzzy miss) against this combined pool. Same-name collisions are resolved by "first-occurrence wins" — higher-priority pool entries shadow lower ones, so a project-specific *"pyrimidine"* deliberately overrides the seed's. Pinning IDs distinguish DB extensions (integer pk string) from seed entries (canonical name); `_resolve_scaffold_pin` tries DB first by integer pk, falls through to seed by canonical name.
- **Executor wiring** in [executor.py](apps/compounds/nlp/executor.py). Both `execute()` (compound path) and `execute_assay_query()` (assay path) thread the resolved Target into `resolve_scaffold`. The compound path uses `rt.registration` (project context comes from the registration target); the assay path uses `resolved_target.target`. Pre-slice-17 callers that don't pass a target still work — they get the shared+seed view.
- **`POST /api/compounds/nlp/scaffold/extend/`** in [view.py](apps/compounds/nlp/view.py). Auth-required, feature-flag-gated (same `COMPOUNDS_NLP_ENABLED` as `/nlp/query`). Validates SMARTS via `RDKit.Chem.MolFromSmarts` before persisting. Returns 400 on parse failure, 409 on same-scope name collision, 201 on success with the created entry's id and target context. Audit columns are populated automatically (`created_by` from the request user, `created_at` via `auto_now_add`).
- **Frontend "Define this fragment" affordance** in [NLPResults.tsx](apps/compounds/frontend/components/compounds/nlp/NLPResults.tsx). When `MissView` renders a scaffold miss, an inline `DefineFragmentForm` lets the chemist type a SMARTS and add the entry to the **shared** catalog (project-scoped additions need target context plumbing through the miss response — deferred). On success the form replaces itself with a green confirmation: *"Added <name> to the shared catalog. Re-run your query to use it."*
- **Tests**: 12 resolver-extension tests (shared resolution, project-scoped resolution, project-vs-shared override, same-name-different-scope, substring + miss with extensions in the pool, pinning round-trip with DB pks). 10 view-endpoint tests (validation, RDKit SMARTS parse failure, target lookup, conflict, end-to-end round-trip via `resolve_scaffold`). Pre-existing scaffold tests gain a `pytestmark = pytest.mark.django_db` since the resolver now consults the DB even when extensions are empty.

What's deliberately out of slice 17:

- **LLM-proposed SMARTS UX** (the chemist clicks Define → backend asks the LLM for a candidate → user reviews + confirms). That's slice 18, where the LLM-as-authority risk needs the visual-review safety rail (RDKit-rendered SVG of the proposed fragment).
- **Per-project additions inline.** The model + endpoint support per-project entries (the `target_id` field is honoured), but the inline form on `ScaffoldMiss` only offers shared scope. Per-project would need the resolved Target's id threaded through the miss payload — small extension, deferred.
- **Curator review queue.** No moderation step in v1 — any authenticated chemist can grow the shared catalog. If abuse becomes a concern, an "approved" boolean + curator UI lands as a follow-up.
- **Django admin registration.** The compounds app doesn't currently use Django admin (no `admin.py` files). Curators edit ScaffoldExtension rows via direct DB access or the future admin UI mentioned above.

**Slice 18 — Same-name User+Supplier union chip** (318 tests cumulative; +8 for the slice)

Slice 16's de-dup keyed on `Supplier.user_id` (the user-linked-supplier case). When the FK is null and the names just *happen* to match — *"Hannah Stewart"* the User who clicked Register three times AND *"Hannah Stewart"* the unlinked Supplier supplying two more compounds — the chemist saw **two chips for the same person** asking which role to filter on. That's the wrong question: the intent is *"compounds Hannah is responsible for"*, which is the union of registered + supplied + via-user-linked-supplier.

- **`UnionCandidate` + `ResolvedUnion`** in [spec.py](apps/compounds/nlp/spec.py). Discriminator `kind="union"` with both `user_id` and `supplier_id` on the wire. Subtitle shows *"3 registered + 2 supplied"* so the merger is visible to the chemist before they click.
- **Cross-tier merger** in [resolver.py](apps/compounds/nlp/resolver.py)'s `_merge_same_name_pairs`. Annotates each User and Supplier with their tier (exact / partial), maps Users by normalised full-name, and walks Suppliers looking for matches. Merged unions take the *stronger* tier of either side, so a User who exact-matches *"Hannah"* via first name AND a Supplier whose full name *"Hannah Stewart"* substring-matches still merge — and the resulting chip is rendered in the exact tier (where the chemist's eye lands first).
- **`resolve_registrant` integration**: if exactly one match remains across all tiers and it's a union, return `ResolvedUnion` directly (single-resolution path). Otherwise mixed-clarify with `UnionCandidate` chips at the top, followed by remaining solo-User and solo-Supplier chips. Pinning continuation: when both `pinned_user_id` AND `pinned_supplier_id` are set on the request, the resolver returns `ResolvedUnion` directly without re-running the matcher.
- **Q-filter** in [executor.py](apps/compounds/nlp/executor.py). The union case ORs all three paths: `Q(registered_by=user) | Q(supplier__user=user) | Q(supplier=sup)`. The executor handles `ResolvedUnion` alongside `ResolvedUser` and `ResolvedSupplier` — three distinct branches.
- **Scope sentence** echoes back as *"registered or supplied by Hannah Stewart"* — distinct from the User branch's *"registered by"* and the Supplier branch's *"from Enamine"*.
- **Frontend wiring**: `applyClarifyPick` dispatches on `candidate.kind === 'union'` to set both pin fields. `CandidateLabel` renders the union chip with the merged-counts subtitle. The clarify-heading remains *"Which person or supplier did you mean by 'X'?"* — works for the union case too.

Tests: 5 resolver tests (single-resolution union, count-preservation in clarify, name-mismatch-no-merger, user-linked-supplier-still-handled-by-slice-16, pinned-union round-trip); 3 executor tests (filter returns union of 5 compounds, scope sentence wording, pinned-continuation round-trip). **318 passing** (was 310).

What's deliberately out of slice 18:

- **LLM-proposed SMARTS for scaffolds** — that was the original slice-18 plan (visual review safety rail for catalog-grow); the Hannah-Stewart bug took priority. LLM-SMARTS becomes slice 19.
- **Triple-merger** (User + user-linked Supplier + same-name unlinked Supplier all at once). Vanishingly rare and would over-complicate the merger logic.
- **Auto-merge by partial name match** (e.g. last-name only). Deliberately rejected — too high a risk of collapsing legitimately different "Stewart" entities.

**Slice 19 — "Best X compounds" ranking by scorecard** (350 tests cumulative; +32 for the slice)

A chemist asking *"best EGFR compounds"* has a clear intent: top-N compounds for the EGFR programme, ranked by *what makes a compound good for EGFR* — which is exactly what the project's spider/radar **scorecard** encodes. Slice 19 maps the *"best"* / *"top N"* phrasing onto a server-side ranking step, applied AFTER all narrowing predicates.

- **Two new selector fields** in [spec.py](apps/compounds/nlp/spec.py): `rank_by: Optional[str]` (only `"scorecard"` in v1; future-proofing for `"potency"` / `"adme"` etc.) and `rank_top_n: Optional[int]` (default 20). Plus `FIELD_RANK_BY` and `RANK_BY_SCORECARD` / `DEFAULT_RANK_TOP_N` constants.
- **Python scorecard evaluator** in [scorecard_rank.py](apps/compounds/nlp/scorecard_rank.py). Mirrors the frontend's [scorecard.ts](apps/compounds/frontend/lib/compounds/scorecard.ts) exactly — same per-axis value extraction (`protocol`, `ratio`, `worst_of`, `lipinski`), same target/poor normalisation with the log-scale default for non-Lipinski axes. Returns mean-of-non-null per-axis t-values per compound. `scorecard_data_needs(config)` extracts the protocol_ids + molecular-property names a config depends on so the aggregator request stays focused.
- **Executor `_apply_ranking`** in [executor.py](apps/compounds/nlp/executor.py). Validates `rank_by`, picks the registration-or-assay target as the scorecard anchor, requires that target to have a `scorecard_config` (otherwise SpecError with a useful nudge — *"Target 'EGFR' has no scorecard configured"*). Calls `aggregate_compact_from_compounds` to produce per-compound rows, evaluates the scorecard, sorts by score descending with stable secondary key, trims to top-N. Compounds with no signal (every axis null) drop from the ranking rather than appearing at the bottom.
- **Scope sentence** prefixes *"Top 20 by scorecard, "* before the existing predicate prose so the chemist sees the ranking as the headline of the result.
- **LLM contract** in [llm.py](apps/compounds/nlp/llm.py). Schema gains `rank_by` + `rank_top_n` (both nullable). System prompt teaches *"best X"* / *"top N X"* with concrete mappings, plus the rule that ranking by anything other than scorecard stays null in v1. *"Show me the top 50 EGFR compounds"* parses naturally — the *"show me"* filler is already handled by existing filler-noun discipline.
- **Frontend**: type-only update on `nlp-api.ts` (`rank_by?: string | null` + `rank_top_n?: number | null` on `CompoundSelector`). The redirect URL still goes through `/assays/aggregate` with the already-ordered compound list — the aggregation page renders what it gets. No UI work required for v1; the rank order is stable in `compound=` URL param.

Tests: **22 scorecard-rank unit tests** (per-axis value extraction across all four kinds, log + linear normalisation with edge cases, mean-t aggregation, scorecard_data_needs across kinds); **7 executor end-to-end tests** (orders compounds by potency, caps to top-N, default 20, composes with threshold filter, scope sentence text, no-scorecard SpecError, drops compounds with no signal); **golden test diff** + **4 new golden entries** ("best CDK4 compounds", "top 10 ARd", "best ARd with HTRF IC50 < 10 nM", "show me the top 50 EGFR"). **350 passing** (was 318).

What's deliberately out of slice 19:

- **Other rank metrics.** *"Best by potency"* / *"Highest IC50"* — v2. The scorecard captures project-defined merit; bespoke rank-by metrics need their own selector vocabulary.
- **Aggregation-page sort/limit URL params.** Server-side ranking returns the already-ordered top-N, so the page doesn't need to know about ranking. If chemists later want to interact with the ranked list (e.g. "show me #21–40 next"), pagination at the page layer is the natural extension.
- **Per-target rank-by configuration.** Targets configure scorecards already; "best EGFR" reuses that. No new config story needed.

**Slice 20 — URL-length fix via token-addressed Selection** (368 tests cumulative; +18 for the slice)

Browser URL-length limits are ~2 KB safe / ~8 KB tolerated. At ~13 chars per `NCL-XXXXXXXX,` an NLP redirect inlining `?compound=...,...,...` started to break around 150 compounds — and *"all CDK4 compounds"* in a mature programme blows through that easily, while slice 21's similarity feature will routinely return hundreds. Pre-slice-20 the redirect silently truncated or 414'd; chemists wouldn't immediately know why.

The fix is the **POST-Redirect-GET** pattern:

- New [`Selection` model](apps/compounds/registry/models.py) — UUID PK, `name` (the scope sentence for round-trip display), `compound_ids` (JSON list of formatted IDs, same shape as the existing `?compound=` URL param), `created_by`, `created_at`, `expires_at`, `source_prompt` (audit). Migration `0025_selection.py`.
- New [endpoints](apps/compounds/registry/selection_views.py) — `POST /api/compounds/selections/` creates a row and returns the token; `GET /api/compounds/selections/<uuid>/` retrieves. Auth required. Creator-only read (404 for other users — don't leak existence). 410 Gone when expired.
- **Threshold switch in the NLP view** ([view.py](apps/compounds/nlp/view.py)). When `len(compound_formatted_ids) > SELECTION_TOKEN_THRESHOLD` (100), `_build_redirect_url` persists a Selection on behalf of the requesting user and emits `?selection=<uuid>`. Below the threshold it inlines as before. Same redirect-URL-building call site, two output shapes.
- **Aggregation page** reads `?selection=<token>` via `useCompoundsApi().get('selections/<id>/')` and treats the resolved `compound_ids` identically to the inline `?compound=` shape downstream. The `PredicateBuilder` is gated behind a "Loading selection..." state until the fetch resolves, so it mounts with the right `initialCompoundSearch`. A `selectionError` alert covers the expired/cross-user-token case.
- **Cleanup management command** at `compounds/registry/management/commands/cleanup_selections.py` — deletes expired rows; `--dry-run` for preview. Run nightly via cron / Azure scheduled job.

What's deliberately out of slice 20:

- **Saved selections** (`is_saved` flag, name/rename, persist forever). Slice 22 adds those alongside the per-user / per-project sharing scope and the NLP-panel-side "this session's selections" list. The current model has just enough fields for the URL-length fix (`expires_at` is nullable so saved selections in slice 22 just null it).
- **Selection composition in queries** (*"Selection 1 AND Selection 2"*). Same slice-22 territory — needs a resolver, a name-uniqueness story, and prompt-engineering for the LLM to recognise "Selection N" / "my hits" phrasings.
- **Assay-list `?ids=` URL** has the same length problem on the assay-selection redirect path. Deferred until either eval data or a real chemist hitting the limit makes it a priority.

**Slice 21 — "Compounds similar to X" via Tanimoto on Morgan fingerprints** (409 tests cumulative; +24 for the slice)

The third entry in the §19.4 resolver family (after compound-by-ID and scaffold). The mechanism: anchor compounds resolved deterministically via `resolve_compound_ref` (slice 14, reused), pre-computed Morgan/ECFP4 fingerprints loaded from `MolecularProperties.morgan_fp`, candidate compounds filtered in Python via `DataStructs.TanimotoSimilarity`. UNION semantics across multiple anchors — *"similar to A or B"* — because that's the natural chemist phrasing; intersection (the bridge case) deferred until eval data justifies the extra vocabulary.

- **Fingerprint at registration** in [registry/models.py](apps/compounds/registry/models.py). `MolecularProperties.calculate_for_compound` (already triggered from `Compound.save()` on SMILES change) gains a Morgan-FP step alongside the existing descriptor calculations. Stored as a 2048-character bit string in a `TextField` — same column whether SQLite or Postgres.
- **Why bit string, not `BinaryField` + `ToBinary()`**: RDKit's `ToBinary()` does sparse compression that DOES NOT preserve the bit-vector length on round-trip via `CreateFromBinaryText` — comparing two unrelated fingerprints raises *"BitVects must be same length"*. `ToBitString()` / `CreateFromBitString()` is a clean round-trip; ~2 KB per compound (low MB at DDU scale, fine).
- **Backfill management command** at `compounds/registry/management/commands/compute_fingerprints.py` — fills in fingerprints for compounds registered before slice 21, plus rows where the SMILES previously failed to parse. `--dry-run` to preview, `--recompute` to overwrite (after RDKit upgrades), `--limit N` for incremental backfills.
- **Tanimoto helper** at [nlp/similarity.py](apps/compounds/nlp/similarity.py). `neighbours_within_threshold(anchors, candidate_pks, threshold) -> set` loads the candidate fingerprints in one batched DB query, deserialises each via `CreateFromBitString`, and keeps compounds whose maximum-Tanimoto across the anchor list is ≥ threshold. Compounds with no fingerprint are silently dropped (analogue of the slice-19 "no signal" case — backfill the catalogue with `compute_fingerprints` rather than try to give them an artificial score).
- **Executor integration** in [executor.py](apps/compounds/nlp/executor.py). Narrowing happens after scaffold but before measurement filters — same compound-level-predicate position. First unresolvable anchor short-circuits to `CompoundMiss` (same shape as `compound_refs_as_typed`). Scope sentence echoes back as *"similar to NCL-00026007 (Tanimoto ≥ 0.5)"*.
- **LLM contract** in [llm.py](apps/compounds/nlp/llm.py). Schema gains `similar_to_as_typed` + `similar_threshold`. System prompt teaches the *"similar to X"* / *"X analogues"* / *"compounds like X"* phrasings, plus the threshold extraction (`Tanimoto > 0.5` → `similar_threshold: 0.5`; `more than 80% similar` → `0.8`). Prompt explicitly distinguishes pinning (compound is added to result) from similarity (compound + neighbours are filtered to result).

Tests: **9 fingerprint + Tanimoto unit tests** (computed on save, recomputed on SMILES change, invalid SMILES doesn't crash, self-similarity = 1, unrelated below threshold, close analogues above, missing-anchor returns empty, multi-anchor union, backfill command). **5 executor end-to-end tests** (similarity narrows, default threshold, unknown anchor → CompoundMiss, composes with measurement filter, scope sentence echo). **5 golden entries** (single anchor, threshold, multi-anchor, similarity + threshold filter, *"analogues"* phrasing).

What's deliberately out of slice 21:
...
**Slice 23 — Scatter view + selection-membership colouring** (cumulative test count: 570; +21 for the slice)

Promotes scatter from a row-pencil-launched Dialog (1234-line `ProtocolScatterPlot`, opened from a Cards/Pivot row icon to scatter that one protocol) to a first-class format mode on the aggregation page, and lets the chemist colour points by membership in any of their saved Selections. Both halves land together because they share a URL contract — `?colour_by=...` is only meaningful in the inline scatter view, and a chip-strip-driven category overlay needs the layout that mode-switching gives it.

- **`OutputFormat` widening** in [types/compounds/aggregation.ts](apps/compounds/frontend/types/compounds/aggregation.ts:63) — `'scatter'` joins `'cards' | 'pivot' | 'bullets' | …` as a peer. The format toggle in `PredicateBuilder` gains a `Scatter` button; the `/assays/aggregate` page parses `?format=scatter` at mount and routes through `AggregationView` to a new branch that mounts `ProtocolScatterPlot` with `mode="inline"`. The dialog mount in `AggregationTable` is preserved (still triggered by the row-pencil "scatter this protocol" flow on Cards/Pivot views).
- **`ProtocolScatterPlot` dual-mode** — new `mode?: 'dialog' | 'inline'` prop. Dialog mode is the back-compat default (Button + MUI Dialog wrapping the chart UI). Inline mode renders the same panel directly (no Button, no Dialog), sized to fill its parent's flex region. The 390-line panel body is shared between modes via a single `panel` JSX const — zero duplication.
- **Selection-categorised datasets** — when `categorisationSelections` (a `Set<formatted_id>` per selection) is non-empty, the chart's single-dataset render path is replaced with one dataset per selection (priority order = the array given) plus an "Other" dataset for unmatched compounds. Chart.js's auto-legend reads each dataset's label, so the legend doubles as the colour key. An 8-colour palette ([CATEGORY_COLOURS](apps/compounds/frontend/components/compounds/ProtocolScatterPlot.tsx)) covers up to 8 simultaneous overlays cleanly. Existing single-dataset assumptions (`pointCount` from `datasets[0]`, `axisBounds` from `datasets[0]`, click-to-navigate via `datasetIndex === 0`) are rewritten to use the stable `_rawPoints` reference.
- **`ScatterCategorisationStrip`** at [components/compounds/aggregation/ScatterCategorisationStrip.tsx](apps/compounds/frontend/components/compounds/aggregation/ScatterCategorisationStrip.tsx). Inline-only chip-strip listing the chemist's saved+ephemeral selections (via `listSelections()`, SWR-cached); each chip toggles the selection's id in the `?colour_by=` URL param via `router.replace`. Page-level effect picks up the URL change and refetches the named selections to repaint.
- **`?colour_by=<uuid>,<uuid>` URL contract** — comma-separated Selection UUIDs, the same chip-toggle source NLP redirects emit. The aggregate page parses on mount, fetches each selection's `compound_ids` via `getSelection(id)`, builds `CategorisationSelection[]` ({ id, name, compoundIds: Set }) and passes through `AggregationTable → AggregationView → ProtocolScatterPlot`. Failed lookups (404 / 410 / network) are dropped silently — the chip strip lets the chemist re-pick without an error toast.
- **NLP fields on `CompoundSelector`** ([spec.py](apps/compounds/nlp/spec.py)): `view_format: Optional[str]` (LLM-emitted; `"scatter"` is the only currently-recognised value, others fall through to default cards) and `categorisation_selection_phrases: List[str]` (raw user-typed phrases naming saved selections). System prompt teaches the *"plot/scatter/chart X vs Y"* + *"coloured by Y"* / *"highlighting X and Z"* phrasings, with explicit examples and a clarification overriding the previous "ignore display formats" instruction.
- **Per-user phrase resolution** in [resolver.py:`resolve_selection_names`](apps/compounds/nlp/resolver.py). Three-tier match against the user's own saved+ephemeral Selection rows (exact / substring / Levenshtein-1 fuzzy with `_FUZZY_MIN_QUERY_LEN = 4` floor). Returns ordered, deduplicated list of UUID strings. Unmatched phrases are silently dropped; per-user scope is enforced at the queryset (`filter(created_by=user)`) — anonymous users get back `[]`.
- **`view.py` redirect URL emission** — the executor sets `CompoundSelection.view_format` (pass-through from the selector); the view layer resolves `categorisation_selection_phrases → categorisation_selection_ids` against `request.user` after `execute()` returns and before serialisation. `_build_redirect_url` whitelists known `view_format` values (`{cards, compact, pivot, bullets, scatter}`), falls back to `cards` otherwise, and appends `colour_by=<uuid>,<uuid>` when categorisation IDs are non-empty.
- **Tests** — 16 new resolver tests at [test_resolver_selection.py](apps/compounds/nlp/tests/test_resolver_selection.py) (empty/anonymous, exact, substring, fuzzy, fuzzy-floor, per-user scope, dedup, expiry, saved-trumps-stale-expiry); 5 new view tests at the bottom of `test_view.py` (format=scatter URL emission, unknown-format fallback, phrase→UUID resolution, unmatched-phrase silent drop, per-user-scoped resolution). Frontend covered by type-check.

What's deliberately out of slice 23:

- **Inline NLP-driven axis selection.** *"Plot pIC50 vs MW"* is recognised as scatter intent but doesn't extract axes — the chemist picks via the page's existing pickers. Axis NLP is a slice of its own (LLM has to map literal column names + protocol names against the user's data, which requires a different resolver shape).
- **Categorisation by anything other than saved selections.** Target / registrant / scorecard tier as colour sources are queued; selections-only is the cleanest first cut and the chip-strip primitive already handles the UX.
- **Composition queries** (*"selection 1 AND selection 2"*) — slice-22 follow-up. The colour-by overlay is per-selection but doesn't combine the populations themselves.
- **Persistent legend reordering / hide.** Chart.js's default legend is read-only. A click-to-hide-dataset interaction is queued.

**Slice 24 — Chemotype categorisation + on-the-fly SMARTS** (cumulative test count: 570; +3 view tests, scaffold matching covered by type-check + manual smoke)

Slice 23's saved-selection-membership colouring shipped first because it was the natural extension of the slice-22 saved-selection infrastructure, but the typical scatter use-case is *"all compounds tested in both protocols, coloured by which chemotype they carry"*. Slice 24 supersedes the categorisation source: the chip strip now lists **scaffolds**, not selections, and the URL/NLP contract emits canonical scaffold names. The saved-selection plumbing is parked (not deleted) for a future *shape_by* axis where selection membership becomes marker shape — giving the scatter quasi-3D information density (X, Y, colour, shape).

- **Substructures listing endpoint** at [view.py: `nlp_substructures_list`](apps/compounds/nlp/view.py) — `GET /api/compounds/nlp/substructures/?target_id=<uuid>` returns `{name, aliases, smarts, source}[]` over the curated seed + project-scoped + shared `ScaffoldExtension` rows. The frontend caches via SWR.
- **Client-side RDKit matching** at [scaffold-matching.ts](apps/compounds/frontend/lib/compounds/scaffold-matching.ts). `buildScaffoldMatches(rdkitModule, scaffolds, compounds)` parses each scaffold SMARTS once via `get_qmol`, then runs `mol.get_substruct_match` per compound × scaffold. Returns `Map<scaffoldName, Set<formattedId>>`. Sub-100ms for ~50 scaffolds × ~500 compounds; no per-compound round-trip. The page memoises so re-renders don't re-match.
- **Chip strip** ([ScatterCategorisationStrip.tsx](apps/compounds/frontend/components/compounds/aggregation/ScatterCategorisationStrip.tsx)) chips are scaffolds from the catalog. Default visibility filters to those with ≥1 matching compound in the current scatter (otherwise the strip drowns in irrelevant chips); a "Show full catalog" toggle exposes the rest. Each chip's tooltip shows the matching count and source.
- **On-the-fly SMARTS** — a "+ SMARTS" input on the strip lets the chemist type any SMARTS pattern; submission appends `smarts:<pattern>` to `colour_by` in the URL. Ad-hoc patterns are matched alongside catalog scaffolds (same `buildScaffoldMatches` pipeline). Comma in SMARTS is rejected at the input boundary (URL contract uses `,` as entry separator). Ad-hoc chips render with a delete icon for explicit removal.
- **URL contract**: `?colour_by=<entry>,<entry>` where each entry is either a canonical scaffold name (`pyrimidine`) or `smarts:<pattern>` (`smarts:c1ccncc1`). The page derives `categorisationSelections` from this URL plus the matches map; the chart's existing dataset-split logic colours points unchanged.
- **NLP layer**: `CompoundSelector.colour_by_scaffold_phrases: List[str]` (LLM-emitted typed phrases). View layer maps each via `lookup_scaffold_by_typed` (slice 13's catalog) to canonical scaffold names; `CompoundSelection.colour_by_scaffolds` flows into the redirect URL. System prompt teaches the *"coloured by pyrimidine"* / *"highlighting indoles vs benzimidazoles"* phrasings; on-the-fly SMARTS isn't NLP-driven (chemists who paste novel patterns are mid-flow on the page).
- **Slice 23's saved-selection plumbing parked**: `resolve_selection_names` resolver and tests stay in place — the dataset-split logic in `ProtocolScatterPlot` is generic over any `Array<{id, name, compoundIds: Set}>` source. Future *shape_by=<uuid>* axis re-uses both.

What's deliberately out of slice 24:

- **Selection-membership as marker shape (third info axis).** Queued for slice 25. The `Map<scaffoldName, Set>` shape generalises to `Map<categoryKey, Set>` so the chart's dataset-split logic doesn't change; only a second visual channel needs wiring.
- **Per-target chemotype catalogs.** The endpoint accepts `target_id` and includes project-scoped extensions, but the frontend doesn't yet pass it. Lands when the aggregation page knows its target context.
- **Match-count badges as percentages.** Counts shown as raw numbers; "X% of scatter compounds carry this scaffold" is an obvious follow-up but not load-bearing.

**Slice 25 — ScaffoldExtension visibility + bug fixes** (cumulative test count: 583; +9 for the slice)

Three fixes shipped together ahead of the DDU→demo handover and the planned compounds-from-ccp4i2 separation pause. All small, all driven by demo-instance feedback.

**25a — colour-by sees ScaffoldExtension rows.** A chemist saved *"Pyrrolopyrimidine"* via the slice-17 "Define this fragment" affordance, then typed *"colour by Pyrrolopyrimidine"* in the NLP bar — and got nothing. The bug was that slice 24's resolution path went through `lookup_scaffold_by_typed`, which **only consults the curated seed catalog**, never the `ScaffoldExtension` DB rows. Meanwhile *"compounds containing Pyrrolopyrimidine"* (slice 13's `scaffold_hints` field) goes through `resolve_scaffold`, which **does** see extensions. One inconsistent resolver, two surfaces. Fix: swap `lookup_scaffold_by_typed` for `resolve_scaffold` in [view.py](apps/compounds/nlp/view.py); the colour-by path now sees project-scoped → shared → seed in the same priority order as `scaffold_hints`. Five-line change, two new tests.

**25b — `/registry/scaffold-extensions` management page.** A chemist couldn't see what was actually stored in their catalog — every reproducer had to be detective work via the conventional substructure search vs the NLP path. New page at [app/registry/scaffold-extensions/page.tsx](apps/compounds/frontend/app/registry/scaffold-extensions/page.tsx) lists every catalog row (seed + extensions) with: name, source chip (Seed / Project / Shared), structure thumbnail (rendered via the existing rdkit-context), **raw SMARTS as stored**, **canonical aromatic SMARTS** that RDKit produces after aromatic perception, the creator's username, and a creator-scoped delete affordance. The split between raw and canonical is the point: a chemist who pasted Kekulé SMILES (`C12=C([N]C=C2)N=CN=C1`) sees the canonical `[#7]:1[cH]:[cH]:c2:c1:n:c:n:c2` (or whichever form `MolToSmarts` produces) and can immediately tell whether normalisation succeeded. Backend: substructures listing endpoint now returns `canonical_smarts` per entry via a SMILES-first → SMARTS-only fallback in `_canonicalise_smarts`; new `DELETE /api/compounds/nlp/scaffold/extensions/<int:id>/` endpoint, creator-scoped, 404 across users (matches the Selection delete contract).

**25c — cards-overflow fallback.** Queries like *"all CDK4 compounds"* land at the cards default but Cards / Pivot / Bullets all render every row to the DOM (no virtualisation). >200 compounds and the page slows or breaks. The redirect URL builder now switches to `compact` (the only genuinely virtualised view) when format ∈ {cards, pivot, bullets} AND result size > 200. Chemist can re-toggle manually. Same threshold as scatter-handles-fine — scatter is one canvas, never affected.

**25d — Kekulé-tolerant chip-strip SMARTS.** The original symptom for the whole slice. `scaffold-matching.ts:buildQueryMol` does a SMILES → canonical aromatic SMARTS → qmol round trip first, falls back to raw SMARTS for genuine SMARTS-only patterns. Pasting Kekulé `C12=C([N]C=C2)N=CN=C1` into the chip strip's "+ SMARTS" now matches aromatic compounds; previously the literal-character query matched zero.

What's deliberately out of slice 25:

- **PATCH on ScaffoldExtension** — edit-by-delete-and-recreate is fine for the management page's first cut. Inline rename / SMARTS edit is queued.
- **Selection management page.** Slice 22's `/selections` CRUD endpoints exist; a parallel page mirroring the scaffold-extensions one is the natural slice 26 — same "show what's stored, edit, delete" pattern.
- **Server-side canonicalisation on save.** `nlp_scaffold_extend` still stores the raw input verbatim. The listing endpoint canonicalises on read; storing canonical at write time would normalise future entries but isn't required for matching to work (the read-side fix already does).
...
| 17 | Python seed stays where it is; DB extension table layers on top | The seed is curated, version-controlled, peer-reviewed — that's the right home for *"pyridine"* / *"sulfonamide"* / *"phenyl"* and the rest of the canonical chemistry vocabulary. The DB table is for runtime growth: project-specific nomenclature (*"Series 1"*), one-off additions (*"benzofuran"*), and curator-blessed shared entries that haven't yet made it into the source-controlled module. Two-source pool with project-priority resolution preserves both invariants. |
| 17 | Same-name override semantics: project beats shared beats seed | A project-specific entry deliberately CAN override a shared entry, and a shared entry can deliberately override the seed. This is the natural consequence of priority resolution and gives curators a soft-override path: if the seed's *"pyrimidine"* SMARTS turns out to be subtly wrong for a programme, the curator adds a project-scoped entry without touching the source-controlled module. The shadowing is documented in `_combined_scaffold_pool`'s docstring so the behaviour is discoverable. |
| 17 | Pinning IDs disambiguate DB pks (numeric strings) from seed canonical names (non-numeric strings) | The slice-13 convention has seed scaffolds use the canonical name as the id (*"pyrimidine"*, *"sulfonamide"*). DB rows have integer pks. Since canonical names are always non-integer strings, `pin_id.isdigit()` is a reliable discriminator: digit → DB lookup; non-digit → seed lookup. Avoids a discriminator field on the wire while keeping pinning unambiguous. |
| 17 | `unique_together = (name, target)`, not just `name` | The same name in two projects is two different fragments (*"Series 1"* in ARd vs CDK4). Constraining uniqueness on the (name, target) tuple is what makes per-project nomenclature possible at all — global uniqueness would force chemists to disambiguate names across projects, defeating the purpose. |
| 17 | RDKit SMARTS validation on the server, not the client | A chemist might fat-finger a SMARTS and the client could happily POST it. Server-side `MolFromSmarts` is the only place we can guarantee the SMARTS parses before persisting; failure returns a structured 400 with `field: "smarts"` so the frontend can surface a useful error inline. RDKit is already a server dep (used by the existing on-the-fly substructure matcher), so no new dependency. |
| 17 | Inline form on ScaffoldMiss is shared-only in v1 | Per-project requires plumbing the resolved target id through the miss payload, which means executor + view changes for a UX detail. Shared-only is the 80% case (most extensions are general chemistry vocabulary, not project nomenclature) and ships the durable architecture cleanly. Per-project gets a richer UI in a follow-up slice once we know how chemists actually want to scope. |
| 18 | Same-name merging is by **normalised full name only**, not last-name or partial | Last-name match would collapse Hannah Stewart (User) and Hannah Stewart (Supplier) — correct — but ALSO Charlie Smith (User) and Bob Smith (Supplier) when neither knows the other. The full-name-match constraint is conservative: it requires both first and last name to align after normalisation, which is what a chemist actually means when they say *"the same Hannah Stewart"*. Suppliers without a parseable "First Last" name (e.g. "Enamine") never merge — they're vendors, not people. |
| 18 | Cross-tier merger: a User in `user_exact` and a Supplier in `sup_partial` (different tiers) still merge if names align, with the merged chip taking the stronger tier (`exact`) | The chemist's intent doesn't depend on whether the resolver matched the User via first-name-exact or the Supplier via substring-of-full-name — both are *"this person"*. Without cross-tier merging, the bug only fixes when the chemist types the full name; typing just first name leaves them with two chips again. The stronger-tier rule keeps the merged chip prominent rather than burying it in the partials. |
| 18 | Both-pin-set is the wire-level signal for `ResolvedUnion` continuation | The pinning round-trip needs to convey union choice with pre-existing schema. Adding a `pinned_union_id` field would have meant a third nullable id; using `(pinned_user_id, pinned_supplier_id)` together is denser and uses what's already there. The resolver's pinning path inspects both first and shortcuts to `ResolvedUnion` if both are set. |
| 18 | Composite candidate `id` (`u:{pk}+s:{pk}`) is opaque | The wire-format id is informational only — the frontend reads `user_id` and `supplier_id` directly off the candidate to populate the two pin fields. The composite string is just a stable React key for the chip render and a debugging aid. Encoding both pks in one id keeps the candidate type uniform with the others (`id` is always present). |
| 18 | Scope sentence reads *"registered or supplied by"*, not *"registered or from"* | *"From"* reads naturally for an external vendor — *"compounds from Enamine"*. *"Or supplied by"* reads naturally for a person — *"compounds registered or supplied by Hannah Stewart"*. The two phrasings target different intuitions about agency: a vendor is a source, a person has a role. |
| 19 | Server-side ranking, not aggregation-page-side | Either the NLP layer returns *"all matching compounds, redirect with sort hint"* and the page sorts/limits, or the NLP layer returns the already-ordered top-N and the page renders what it gets. Server-side keeps the NLP response self-describing (*"Found 20 compounds, top 20 by scorecard"* — the count IS the answer), and the aggregation page stays unaware of `rank_by` semantics. The cost is a Python port of the scorecard evaluator (~80 lines, mirrors the TS module), which is small enough to be worth it. |
| 19 | Ranking metric is the **mean of per-axis t-values**, not weighted or geometric | Each axis contributes equally to "what's good" — the spider visualisation already encodes this convention. Weighted averages would need per-target weight configuration, which doesn't yet exist; geometric mean would over-penalise compounds with one weak axis. Mean-of-t-values is the simplest interpretation that aligns with the existing spider rendering. |
| 19 | No scorecard configured → SpecError, never silent fallback | A chemist typing *"best EGFR compounds"* against a target with no scorecard hits a clean error (*"configure a scorecard for this project, or use an explicit threshold"*). The alternative — silently rank by HTRF IC50 (or whatever) — would be undocumented magic that produces different rankings depending on which protocols happen to exist. The error nudges the chemist towards either configuring the scorecard (the durable answer) or rephrasing without *"best"*. |
| 19 | Compounds with no signal (every axis null) are **dropped** from the ranking, not sorted to the bottom | Either they sort to the bottom (where their position is meaningless because there's no signal), or they vanish from the ranked list entirely. Vanishing is honest: *"top 20"* should mean *"top 20 with measurable scorecard performance"*. If the chemist wants to see *"compounds with no measurements yet"* they'll ask for that specifically. |
| 19 | Mean-t scorecard evaluator is a **Python port** of the TS module, not a thin wrapper | A round-trip via the aggregation API + frontend evaluation would couple the NLP backend to client-side execution. The Python port is ~80 lines, well-bounded, and lets the NLP path stay server-only. Drift between the TS and Python implementations is the main risk; mitigated by exhaustive unit tests on the Python side that mirror the TS module's behaviour at every branch (per-axis-value, normalisation, mean aggregation, data-needs extraction). |
| 19 | Default top-N is 20 | A screen-sized slice. Chemists who want fewer say so (*"top 5 ARd compounds"*); chemists who want more say so (*"top 100"*). Twenty is what fits comfortably on a project-card view at typical zoom levels. |
| 20 | Token threshold of 100 compounds, not 150 | The browser URL safe limit is ~2 KB which works out to ~150 `NCL-XXXXXXXX,` entries. Setting the threshold conservatively at 100 leaves headroom for the rest of the URL params (`format`, `targets`, `protocols`) and for compound IDs longer than 12 characters in non-NCL deployments. False positives (a Selection row created for 110 compounds when 150 would have inlined fine) are cheap; false negatives (URL truncation in the wild) are user-visible bugs. |
| 20 | Selection-id is the storage key, not e.g. a hash of the compound list | A hash would let identical selections share a row and avoid duplicate writes. But it would also mean two chemists running the same prompt at different times share an `expires_at` (whoever cleaned up first invalidates the other's URL), and the audit trail would conflate distinct invocations. UUIDs keep each NLP run's selection independent — cheap inserts, clean lifecycle, separate audit. |
| 20 | 404 (not 403) when a different user tries to read a selection | Don't leak the existence of selections across users — an attacker probing the UUID space sees the same 404 whether the row exists or not. Same discipline as the rest of the compounds API. |
| 20 | Expired selections return 410 Gone, not 404 | Distinguishing "didn't exist" (404) from "did exist, no longer accessible" (410) is the HTTP idiom for resource lifecycle. Lets the frontend show a useful "this selection has expired — try re-running the query" message rather than the generic not-found case. |
| 20 | Aggregation page **gates** the PredicateBuilder behind selection-fetch completion, rather than initialising it twice | The PredicateBuilder consumes `initialCompoundSearch` only on first mount (in `useState` initialisers). If we mounted it eagerly with the URL pending, it would lock in an empty search and ignore the resolved selection. The loading-state gate is a four-line change vs. retrofitting effect-driven re-initialisation through several state hooks downstream. |
| 20 | Selection model lives in `registry/`, not in a new `selections/` app or a sidecar module | The model is conceptually adjacent to Compound (a snapshot of the registry), and putting it with the existing migration history avoids one more app to register. Slice 22 may grow it (saving / sharing / composition), but the home in `registry/models.py` stays right because the entity is still a registry concept. |
| 21 | Fingerprint stored as a 2048-character bit string, not bytes | `RDKit.ExplicitBitVect.ToBinary()` does sparse compression and DOES NOT preserve the bit-vector length on round-trip via `CreateFromBinaryText`. Comparing two such reconstructed fingerprints raises *"BitVects must be same length"*. `ToBitString()` / `CreateFromBitString()` is a clean round-trip; ~2 KB per compound is fine at low-MB total. Cleanly portable across SQLite / Postgres without engine-specific code. |
| 21 | UNION across multiple anchors, not intersection | *"compounds similar to A or B"* is the natural chemist phrasing; *"similar to A and B"* (intersection — the SAR bridge case) is rarer in practice and gives empty results unless the anchors are themselves close. Default to the broader, more useful interpretation; revisit if eval data shows chemists wanting intersection. |
| 21 | Compounds with no fingerprint are silently dropped from the result | Same discipline as slice 19's no-scorecard-signal case: a compound that can't be scored / compared shouldn't appear in a "best 20" or "20 most similar" result with an artificial 0. The escape valve is the `compute_fingerprints` management command — backfill the catalogue, then queries that depend on the fingerprint catch the previously-missing rows. |
| 21 | Anchor with no fingerprint → empty result, not "match everything" | A defensive failure mode: if the anchor's fingerprint is missing (backfill not yet run on a pre-existing compound), naively returning the un-narrowed candidate set would be a silent miss that looks like "everything is similar to X". Returning empty surfaces the issue: the chemist sees zero results and the curator runs the backfill. |
| 21 | Threshold default is 0.7 | The de-facto medicinal-chemistry "structurally similar" cutoff. Below 0.5 is "vaguely related"; ≥0.85 is "essentially the same compound". 0.7 is "looks like the same series" — what a chemist asking for *"similar"* compounds typically means without further qualification. |
| 21 | Tanimoto computation is a Python loop, not a SQL aggregate | DDU-scale: low-thousands of compounds → sub-second Python loop. The Postgres RDKit cartridge would let `WHERE morgan_fp % anchor > 0.7` run in the database engine — a meaningful win at 10K+ compound libraries — but requires the cartridge to be installed on the deployment's Postgres instance, which is a deployment-level decision. Defer until either eval data or a real performance complaint warrants it. |
