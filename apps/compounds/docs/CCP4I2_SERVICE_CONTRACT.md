...
- **Draft.** Land for review with CCP4 dev team before any external party builds against it.
- TypeScript types live in [`packages/ccp4i2-auth/src/contracts/ccp4i2.ts`](../../../packages/ccp4i2-auth/src/contracts/ccp4i2.ts) and re-export from `@ccp4/ccp4i2-auth`. External consumers `import type { Project, Job, File, ... } from "@ccp4/ccp4i2-auth"`.
- Companion to the other strategy docs in this folder ([proposal](CCP4I2_FACING_PROPOSAL_REPO_SPLIT.md), [implementation plan](REPO_SPLIT_IMPLEMENTATION.md)). Step 4 of the proposal sequencing.
