...
JOB_STATUS_TEXT = ['Unknown', 'Pending', 'Queued', 'Running', 'Interrupted', 'Failed',
                   'Finished', 'Running remotely', 'File holder', 'To delete', 'Unsatisfactory']

FINISHED_JOB_STATUS = ['Finished',
                       'Interrupted', 'To delete', 'Unsatisfactory']
...
FILETYPELIST = [(0, 'Unknown', 'File type unknown'),
                ...
                (6, 'application/CCP4-unmerged-experimental',
                ...
                (11, 'application/CCP4-mtz-observed',
                 'Observed intensities and structure factors'),
                ...
                (18, 'application/refmac-external-restraints',
                 'Refmac external restraints'),
                ...
                (26, 'application/CCP4-HHPred-alignments',
                 'HHPred sequence search results'),
                (27, 'application/CCP4-Blast-alignments',
...
class CDbApi(CObject):
    ...
    DATABASEITEMS = ['databaseid', 'creationtime', 'creationhostname',
                     'creationusername', 'schemaversion', 'schemadate']
    ...
    PROJECTTYPES = [UUIDTYPE, str, float, UUIDTYPE,
                    UUIDTYPE, int, str, UUIDTYPE, float, str, str, float]
    # used in getTablesEtree..
    PROJECTITEMS0 = ['projectid', 'projectname',
                     'projectcreated', 'lastjobnumber', 'projectdirectory']
    ...
    FILEITEMS = ['fileid', 'filename', 'jobid', 'jobparamname',
                 'filetypeid', 'annotation', 'filesubtype', 'filecontent', 'pathflag']
    FILETYPES = [UUIDTYPE, str, UUIDTYPE, str, int, str, int, int, int]

    IMPORTFILEITEMS = ['importid', 'fileid', 'creationtime', 'lastmodifiedtime', 'checksum',
                       'sourcefilename', 'sourcefileid', 'exportfileid', 'annotation', 'importnumber', 'reference']
    IMPORTFILETYPES = [UUIDTYPE, UUIDTYPE, float, float,
                       str, str, UUIDTYPE, UUIDTYPE, str, int, str]

    EXPORTFILEITEMS = ['exportid', 'creationtime',
                       'fileid', 'exportfilename', 'annotation']
    ...
    PROJECTCOMMENTITEMS = ['projectcommentid',
                           'projectid', 'userid', 'timeofcomment', 'comment']
    ...
    FILEASSOCIATIONITEMS = ['fileassociationid',
                            'fileassociationtypeid', 'fileassociationname']
    FILEASSOCIATIONTYPES = [UUIDTYPE, int, str]

    FILEASSOCIATIONMEMBERITEMS = [
        'fileassociationid', 'fileid', 'fileassociationroleid']
    FILEASSOCIATIONMEMBERTYPES = [UUIDTYPE, UUIDTYPE, int]

    PROJECTIMPORTITEMS = ['projectimportid', 'projectid', 'projectimporttime',
                          'projectexportid', 'projectexportdatabaseid', 'projectexporttime', 'projectexportafter']
    PROJECTIMPORTTYPES = [UUIDTYPE, UUIDTYPE,
                          float, UUIDTYPE, UUIDTYPE, float, float]
    ...
    def __init__(self, parent=None, mode=None, fileName=None, hostName='localhost', userName=None, userPassword=None, createDb=False, **kw):
        ...
        if fileName is None:
            if mode in ['sqlite', 'qt_sqlite']:
                ...
                fileName = os.path.join(
                    CCP4Utils.getDotDirectory(), 'db', 'database.sqlite')
        ...
        self._preferences = {'testFileExists': True,
                             'useTransaction': False, 'useImportFileDir': True}
        self._conn = None
        self._cur = None
        self._open = False
        ...
        self._followFromJob = {}
        self.openDb(fileName=fileName, createDb=createDb,
                    diagnostic=kw.get('loadDiagnostic', True))
    ...
    def loadFileTypes(self, diagnostic=True):
        ...
        if fmax < len(FILETYPELIST):
            if diagnostic:
                print('Loading new file type into database',
                      FILETYPELIST[fmax:])
            for item in FILETYPELIST[fmax:]:
                self.execute(
                    ' INSERT INTO Filetypes (FiletypeID,FiletypeName,FiletypeDescription) VALUES (?,?,?)', item)
    ...
    def loadJobStatus(self, diagnostic=True):
        ...
        if fmax <= len(JOB_STATUS_TEXT):
            for idx in range(fmax, len(JOB_STATUS_TEXT)):
                if diagnostic:
                    print('Loading new job status into database',
                          idx, JOB_STATUS_TEXT[idx])
                self.execute(
                    ' INSERT INTO JobStatus (StatusID,StatusText) VALUES (?,?)', (idx, JOB_STATUS_TEXT[idx]))
    ...
    def loadKeyTypes(self, diagnostic=True):
        ...
        if fmax < len(KEYTYPELIST):
            if diagnostic:
                print('Loading new key type into database', KEYTYPELIST[fmax:])
            for item in KEYTYPELIST[fmax:]:
                self.execute(
                    ' INSERT INTO KeyTypes (KeyTypeID,KeyTypeName,KeyTypeDescription) VALUES (?,?,?)', item)

    def loadFileAssociationTypes(self):
        self.execute(
            'SELECT MAX(FileAssociationTypeID) FROM FileAssociationTypes')
        fmaxList = self.fetchAll2Py(int)
        if fmaxList[0] is None:
            fmax = 0
        else:
        ...
        if fmax < len(FILEASSOCIATIONTYPELIST):
            print('Loading new file association type into database',
                  FILEASSOCIATIONTYPELIST[fmax:])
            for item in FILEASSOCIATIONTYPELIST[fmax:]:
                self.execute(
                    ' INSERT INTO FileAssociationTypes (FileAssociationTypeID,FileAssociationTypeName,FileAssociationTypeDescription) VALUES (?,?,?)', item)
        self.execute(
            'SELECT MAX(FileAssociationRoleID) FROM FileAssociationRoles')
        fmaxList = self.fetchAll2Py(int)
        if fmaxList[0] is None:
            fmax = 0
        else:
        ...
        if fmax < len(FILEASSOCIATIONROLELIST):
            print('Loading new file association roles into database',
                  FILEASSOCIATIONROLELIST[fmax:])
            for item in FILEASSOCIATIONROLELIST[fmax:]:
                self.execute(
                    ' INSERT INTO FileAssociationRoles (FileAssociationRoleID,FileAssociationRoleName,FileAssociationRoleDescription,FileAssociationTypeID) VALUES (?,?,?,?)', item)
    ...
    def cacheTextLabels(self):
        self.execute(
            "SELECT StatusID,StatusText FROM JobStatus ORDER BY StatusID")
    ...
    def loadSchema(self, fileName=None):
        if fileName is None:
            from core import CCP4Utils
            fileName = os.path.join(
                CCP4Utils.getCCP4I2Dir(), 'dbapi', 'database_schema.sql')
        print('Loading database schema from:',
              fileName, 'Version:', self._schemaVersion)
    ...
    def getDbSchemaVersion(self):
        try:
            self.execute(
                'SELECT SchemaVersion,SchemaDate FROM Databases WHERE ThisIsMe > 0 ORDER BY CreationTime DESC')
    ...
    def validateSchemaVersion(self):
        ...
        if version != self._schemaVersion[0]:
            raise CException(self.__class__, 222, 'Db version: '+str(version) +
                             ' current schema version: '+str(self._schemaVersion[0]))
    ...
    def update_0_1_10(self):
        ...
        self.execute(
            'UPDATE Databases SET SchemaVersion=? , SchemaDate=? WHERE ThisIsMe>0', ('0.1.11', '23-07-2013'))
        self.commit()
    ...
    def update_0_1_11(self):
        ...
        self.execute(
            'UPDATE Databases SET SchemaVersion=? , SchemaDate=? WHERE ThisIsMe>0', (next, date))
        self.commit()
    ...
    def update_0_1_14(self):
        ...
        self.execute(
            'ALTER TABLE Jobs ADD UserId VARCHAR(32) REFERENCES Users (UserId)')
...
#            self.execute('PRAGMA locking_mode = EXCLUSIVE')
            ...
            if not self._conn.isValid():
                try:
                    self._conn = QtSql.QSqlDatabase.addDatabase(
                        'QSQLITE', 'CCP4DB')
                except Exception as e:
                ...
                try:
                    ...
                    raise CException(self.__class__, 142,
                                     self._fileName+'\nError:'+str(e))
                if not self._conn.isValid():
    ...
    def commit(self):
        ...
        if self._dbMode == 'sqlite':
            try:
                ...
                raise CException(self.__class__, 104, str(
                    self._commands)+'\nError:'+str(e))
            self._commands = []
            self._cur = None
        else:
    ...
    def execute(self, com, args=[]):
        ...
        if self._dbMode == 'sqlite':
            ...
            if not self._open:
                ...
                if not rv:
                    raise CException(self.__class__, 152, com +
                                     ' '+str(args), stack=False)
                if self._preferences['useTransaction']:
                    rv = self.connection().transaction()
                    if not rv:
                        raise CException(self.__class__, 153,
                                         com+' '+str(args), stack=False)
                self._open = True
            ...
            if lastError.type() != QtSql.QSqlError.NoError:
                ...
                raise CException(self.__class__, 150, 'Code: ' +
                                 str(lastError.type())+'  '+com+' '+str(args))
                ...
                for ii in range(len(row)):
                    if row[ii] is None:
                        ...
                        try:
                            ...
                            print('ERROR in CCP4DbApi.fetchAll2PyList',
                                  row, toType, ii)
                            import traceback
        ...
        if idName is None:
            ...
            if idName is None:
                if table[-1] == 's':
                    ...
                    idName = table+'ID'
        self.execute('SELECT '+idName+' FROM '+table +
    ...
    def setupBackupJobToXml(self):
        import functools
        self.jobCreated.connect(functools.partial(
            self.backupJobToXml, 'jobCreated'))
        self.jobUpdated.connect(functools.partial(
            self.backupJobToXml, 'jobUpdated'))
        self.importFileCreated.connect(functools.partial(
            self.backupJobToXml, 'importFileCreated'))
        self.importFileUpdated.connect(functools.partial(
            self.backupJobToXml, 'importFileUpdated'))
        self.exportFileCreated.connect(functools.partial(
            self.backupJobToXml, 'exportFileCreated'))
        self.commentEdited.connect(functools.partial(
            self.backupJobToXml, 'commentEdited'))
        self.commentDeleted.connect(functools.partial(
            self.backupJobToXml, 'commentDeleted'))
    ...
    def backupJobToXml(self, signal, args):
        ...
        if signal == 'jobUpdated':
            if not args['key'] in ['preceedingjob', 'title', 'evaluation']:
                return
        jobId = args['jobId']

        jobInfo = self.getJobInfo(
            jobId=jobId, mode=['jobnumber', 'projectname', 'taskname'])
    ...
    def integerId(self, table, identifier):
        self.execute(
            'SELECT LastID FROM LastUniqueIds WHERE TableName=?', (table,))
        rv = self.fetchAll2Py(int)
        if len(rv) == 1:
            newValue = rv[0] + 1
            self.execute(
                'UPDATE LastUniqueIds SET LastID=? WHERE TableName=?', (newValue, table,))
            return newValue
        else:
            args = (identifier, table)
            self.execute('SELECT '+identifier+' FROM ' +
                         table+' ORDER BY '+identifier)
            ...
            if len(l) == 0:
                ...
                newValue = l[-1]+1
            self.execute(
                'INSERT INTO LastUniqueIds (TableName,LastID) VALUES (?,?)', (table, newValue))
            return newValue
    ...
    def createUser(self, userName, userPassword=None, userRole=USER_ROLE_USER):
        if not self._userRole in [USER_ROLE_MANAGER, USER_ROLE_OWNER]:
            raise CException(self.__class__, 106, 'You are ' +
                             self._userName + ' calling createUser')
        try:
    ...
    def updateUser(self, userName, newUserName=None, userPassword=None, userRole=None):
        ...
        if userPassword is not None:
            ...
            self.execute(
                "UPDATE Users SET userPassword= ? WHERE userID= ?", args)
        if userRole is not None:
            ...
            self.execute(
                "UPDATE Users SET userRoleID= ? WHERE userID= ?", args)
        self.commit()
    ...
    def deleteUser(self, userName):
        ...
        if not self._userRole in [USER_ROLE_MANAGER, USER_ROLE_OWNER]:
            raise CException(self.__class__, 106, 'You are ' +
                             self._userName+' calling deleteUser')
        userId = self.getUserId(userName)
    ...
    def createProject(self, projectName, userName=None, parentProjectId=None, projectDirectory=None, projectId=None):
        ...
        if not self._userRole in [USER_ROLE_MANAGER, USER_ROLE_OWNER]:
            raise CException(self.__class__, 106, 'You are ' +
                             self._userName+' calling createProject')
        ...
        matchID, matchName, relpath = self.matchProjectDirectory(
            projectDirectory)
        if matchID is not None and (relpath is None or len(relpath) == 0):
            raise CException(self.__class__, 117, 'Matching: ' +
                             str(matchName)+' Relative path: '+str(relpath), stack=False)
        ...
        try:
            args = (projectId, projectName, time, userId,
                    parentProjectId, 0, projectDirectory, time)
            self.execute(
                "INSERT INTO Projects (ProjectID,ProjectName,ProjectCreated,UserID,ParentProjectID,LastJobNumber,ProjectDirectory,LastAccess) VALUES (?,?,?,?,?,?,?,?);", args)

            args = (projectId, userId, PRIVILEGE_EXTEND, userId)
            self.execute(
                "INSERT INTO ProjectsUsersPermissions (ProjectID,UserID,PrivilegeID,CreatorID) VALUES (?,?,?,?);", args)
        except Exception as e:
            raise CException(self.__class__, 200,
                             'args: '+str(args)+' '+str(e))
    ...
    def getProjectInfo(self, projectId=None, mode='all', projectName=None, checkPermission=True):
        if projectId is None:
            if projectName is None:
                raise CException(self.__class__, 174)
        ...
        if mode == 'lastjobaccesstime':
            self.execute(
                'SELECT MAX (FinishTime) FROM Jobs WHERE ProjectId=?', (projectId,))
            lastFinish = self.fetchAll2Py(float)
            self.execute(
                'SELECT MAX (CreationTime) FROM Jobs WHERE ProjectId=?', (projectId,))
        ...
        if 'parentprojectname' in extraItemList:
            self.execute(
                'SELECT parentprojectid FROM Projects WHERE ProjectID =?', (projectId,))
            rv = self.fetchAll2Py(UUIDTYPE)
            if len(rv) != 1:
                raise CException(self.__class__, 118, str(projectId))
            elif rv[0] is None:
                ...
                ret['parentprojectname'] = self.getProjectInfo(
                    rv[0], 'projectname')
    ...
    def getProjectJobSearchInfo(self, projectId, topOnly=True):
        ...
        if topOnly:
            com = com + ' AND ParentJobId IS NULL'
        self.execute(com, (projectId,))
        taskNameList = self.fetchAll2Py(str)
        self.execute(
            'SELECT MIN(FinishTime) FROM Jobs WHERE Jobs.ProjectId = ? AND ParentJobId IS NULL', (projectId,))
        minTime = self.fetchAll2Py(int)
        self.execute(
            'SELECT MAX(FinishTime) FROM Jobs WHERE Jobs.ProjectId = ? AND ParentJobId IS NULL', (projectId,))
...
TaskTitle TEXT );''')
        ...
        for taskName, title in inputList:
            self.execute(
                'INSERT INTO TempTaskTitles (TaskName,TaskTitle) VALUES (?,?)', (taskName, title))
        self.commit()
    ...
    def projectJobSearch(self, projectId, topOnly=True, searchParams={}):
        ...
        if searchParams.get('taskName', None) is not None:
            if not isinstance(searchParams['taskName'], list):
                ...
                for item in searchParams['taskName'][1:]:
                    com = com+',?'
                com = com + ')'
            ...
            if searchParams.get('textSearchMode', 0) > 0:
                ...
                if searchParams['textSearchMode'] == 2 and searchParams['searchText'][0] != '%':
                    searchParams['searchText'] = '%' + \
                        searchParams['searchText']
            com = com + \
                ' AND ( Jobs.JobTitle LIKE ? OR Comments.Comment LIKE ? OR (Jobs.JobTitle IS NULL AND TempTaskTitles.TaskTitle LIKE ?))'
    ...
    def getProjectSearchInfo(self):
        ...
        self.execute(
            'SELECT MIN (ProjectCreated), MAX(ProjectCreated), MIN(LastAccess),MAX(LastAccess) FROM Projects')
    ...
    def setAllProjectsLastAccess(self):
        ...
        for pid in projectIdList:
            self.execute('UPDATE Projects SET LastAccess = ? WHERE ProjectId = ?',
                         (self.getProjectInfo(pid, 'lastjobaccesstime'), pid))
        self.commit()

    def updateProjectLastAcess(self, closingProjectId=None, openProjectIdList=[]):
        t = self.currentTime()
        self.execute(
            'UPDATE Projects SET LastAccess = ? WHERE ProjectId = ?', (t, closingProjectId))
        for pid in openProjectIdList:
            self.execute(
                'UPDATE Projects SET LastAccess = ? WHERE ProjectId = ?', (t+1.0, pid))
        self.commit()

    def resetLastJobNumber(self, projectId, commit=True):
        self.execute(
            'SELECT JobNumber FROM Jobs WHERE ProjectId = ? AND ParentJobId IS NULL', (projectId,))
        ...
        self.execute('UPDATE Projects SET LastJobNumber = ? WHERE ProjectId = ?', (str(
            maxJobNum), projectId))
        if commit:
            self.commit()
    ...
    def _getChildProjects(self, projectId, recurse=False):
        self.execute(
            'SELECT ProjectID FROM Projects WHERE ParentProjectID =?', (projectId,))
        rv = self.fetchAll2Py(UUIDTYPE)
        if not recurse:
            return rv
        else:
    ...
    def getProjectUnfinishedJobs(self, projectId=None):
        self.execute('SELECT JobId FROM JOBS WHERE ProjectId=? AND Status != ?', (str(
            projectId), JOB_STATUS_FINISHED))
        rv = self.fetchAll2Py(UUIDTYPE)
        return rv
    ...
    def getProjectPermission(self, projectId=None, userName=None, projectName=None, jobId=None):
        ...
        if projectName is not None:
            permission = self._projectPermissions.get(
                userName, {}).get(projectName, None)
        else:
            permission = self._projectPermissions.get(
                userName, {}).get(projectId, None)
        ...
        self.execute(
            'SELECT PrivilegeID FROM ProjectsUsersPermissions  WHERE ProjectID = ? AND UserID = ?', arg)
        rv = self.fetchAll2Py(int)
        if len(rv) == 0:
            raise CException(self.__class__, 108, 'For ' +
                             str(projectId)+':'+str(userName))
        else:
            if userName not in self._projectPermissions:
                self._projectPermissions[userName] = {}
            if projectName is not None:
    ...
    def getProjectJobFileName(self, projectId=None, fileName=None, jobNumber="1", subJobNumber=""):
        if subJobNumber != "":
            fname = os.path.join(self.getProjectDirectory(
    ...
    def getProjectJobFile(self, projectId=None, fileName=None, jobNumber="1", subJobNumber=""):
        fname = self.getProjectJobFileName(
    ...
    def getProjectDirectory(self, projectId=None, projectName=None, jobId=None):
        ...
        if projectId is None:
            if projectName is not None:
                projectId = self.getProjectId(projectName)
            if projectId is None:
                raise CException(self.__class__, 118)

        # print 'getProjectDirectory',projectId,type(projectId)
        arg = (projectId,)
        self.execute(
            'SELECT ProjectDirectory FROM Projects  WHERE ProjectID = ?', arg)
    ...
    def getProjectDirectoryList(self, order='ASC'):
        if not order in ['ASC', 'DESC']:
            order = 'ASC'
        self.execute(
            'SELECT ProjectID,ProjectName,ProjectDirectory,ParentProjectID,ProjectCreated,LastAccess FROM Projects ORDER BY ProjectName '+order)
    ...
    def getRecentProjects(self, order='name', limit=10):
        ...
        self.execute(
            'SELECT ProjectID,ProjectName,LastAccess FROM Projects ORDER BY LastAccess DESC LIMIT ?', (limit,))
    ...
    def getProjectsToCleanup(self, period=86400.0):
        ...
        self.execute(
            'SELECT ProjectID,ProjectDirectory,ProjectName FROM Projects WHERE LastCleanupTime IS NULL OR LastAccess > LastCleanupTime - ?', (period,))
    ...
    def getProjectFollowFromJobId(self, projectId=None):
        ...
        arg = (projectId,)
        self.execute(
            'SELECT followFromJobId FROM Projects  WHERE ProjectID = ?', arg)
        rv = self.fetchAll2Py(UUIDTYPE)
        if len(rv) == 0:
            return None
        else:
    ...
    def setProjectFollowFromJobId(self, projectId=None, jobId=None, clear=False):
        ...
        if permission < PRIVILEGE_WRITE:
            raise CException(
                self.__class__, 112, 'setProjectFollowFromJobId to project: '+str(projectId))
        ...
        previousFollowFromJobId = self.getProjectFollowFromJobId(
            projectId=projectId)
        ...
        if clear:
            if jobId == previousFollowFromJobId:
                args = (None, projectId)
                self.execute(
                    'UPDATE Projects SET followFromJobId= ? WHERE ProjectID= ?', args)
                jobId = None
            else:
                return
        else:
        ...
        self.followFromJobChanged.emit(
            (projectId, previousFollowFromJobId, jobId))
        return

    def considerUpdatingFollowFrom(self, projectId, finishedJobId):
        jobInfo = self.getJobInfo(jobId=finishedJobId, mode=[
                                  'status', 'parentjobid', 'preceedingjobid'])
        if jobInfo['status'] != 'Finished' or jobInfo['parentjobid'] is not None:
            return
        ...
        if followJobId is None:
            ...
            fileTypeList = self.getJobFiles(
                jobId=finishedJobId, mode='fileTypeId')
            # print 'considerUpdatingFollowFrom fileInfo',fileTypeList
            nF = 0
            for fType in [2, 3, 4, 5, 6, 11, 12, 13]:
                nF = nF + fileTypeList.count(fType)
            if nF > 0:
                self.setProjectFollowFromJobId(
                    projectId=projectId, jobId=finishedJobId)
        else:
    ...
    def getFilePermission(self, fileId, userName=None):
        ...
        u = (str(fileId),)
        self.execute(
            'SELECT Jobs.JobID,Jobs.ProjectID FROM Jobs INNER JOIN Files ON Jobs.JobId = Files.JobId WHERE Files.FileID = ?', u)
    ...
    def exportJobSelectionCommand(self, projectId=None, after=None, status=None, jobList=None):
        if jobList is not None:
            ...
            for jobNo in jobNumberList:
                self.execute(
                    "SELECT jobid FROM jobs where projectId = ? AND jobnumber LIKE '"+jobNo+".%'", (projectId,))
            ...
            if status is not None:
                if status == 'exportable':
                    status = [JOB_STATUS_PENDING, JOB_STATUS_INTERRUPTED, JOB_STATUS_FINISHED,
                              JOB_STATUS_FAILED, JOB_STATUS_UNSATISFACTORY, JOB_STATUS_FILE_HOLDER]
    ...
    def getJobsByTime(self, projectId=None, before=None, after=None, mode='JobID', topLevel=False, status=None, jobList=None):
        jobSelectCom, jobSelectArgs = self.exportJobSelectionCommand(
            projectId=projectId, after=after, status=status, jobList=jobList)
    ...
    def getTablesEtree(self, projectId=None, after=None, status=None, jobList=None, inputFileList=None, inputFileFromJobList=None):
        ...
        databaseTableEle = self.tableEtree(
            'database', self.DATABASEITEMS, table)
        root.append(databaseTableEle)

        com = 'SELECT '+self.PROJECTITEMS0[0]
        for item in self.PROJECTITEMS0[1:]:
            com = com + ', '+item
        ...
        info = self.getProjectInfo(projectId=projectId, mode=[
                                   'userid', 'parentprojectname'])
        self.execute('SELECT UserName FROM Users WHERE UserId=?',
                     (info['userid'],))
...
# FIXME - The above may return empty list. Hmm. Seems ProjectComments are not imported properly. Fix here.
        ...
        if info['parentprojectname'] is not None:
            projectTableEle[0].set('parentprojectname',
        ...
        for row in table:
            if not row[idx].count('.'):
                jobNumberList.append(row[idx])
        ...
        if inputFileFromJobList is not None and len(inputFileFromJobList) > 0:
            ...
            for item in inputFileFromJobList:
                com = com + '?,'
            com = com[0:-1]+')'
        ...
        for item in self.FILEUSEITEMS[1:]:
            com = com + ', FileUses.'+item
        ...
        for item in self.IMPORTFILEITEMS[1:]:
        ...
        for item in self.EXPORTFILEITEMS[1:]:
            com = com + ', ExportFiles.'+item
        ...
        for item in self.COMMENTITEMS[1:]:
            com = com + ', Comments.'+item
        ...
        for item in self.PROJECTCOMMENTITEMS[1:]:
            com = com + ', ProjectComments.'+item
        ...
        root.append(self.tableEtree('projectcomment',
                    self.PROJECTCOMMENTITEMS, table))

        com = 'SELECT JobKeyValues.'+self.JOBKEYVALUEITEMS[0]
        for item in self.JOBKEYVALUEITEMS[1:]:
            com = com + ', JobKeyValues.'+item
        ...
        root.append(self.tableEtree('jobkeyvalue',
                    self.JOBKEYVALUEITEMS, table))

        com = 'SELECT JobKeyCharValues.'+self.JOBKEYCHARVALUEITEMS[0]
        for item in self.JOBKEYCHARVALUEITEMS[1:]:
            com = com + ', JobKeyCharValues.'+item
        ...
        root.append(self.tableEtree('jobkeycharvalue',
                    self.JOBKEYCHARVALUEITEMS, table))

        com = 'SELECT DISTINCT FileAssociationMembers.' + \
            self.FILEASSOCIATIONMEMBERITEMS[0]
        for item in self.FILEASSOCIATIONMEMBERITEMS[1:]:
            com = com + ', FileAssociationMembers.'+item
        ...
        root.append(self.tableEtree('fileassociationmember',
                    self.FILEASSOCIATIONMEMBERITEMS, table))

        com = 'SELECT DISTINCT FileAssociations.'+self.FILEASSOCIATIONITEMS[0]
        for item in self.FILEASSOCIATIONITEMS[1:]:
            com = com + ', FileAssociations.'+item
        ...
        root.append(self.tableEtree('fileassociation',
                    self.FILEASSOCIATIONITEMS, table))

        com = 'SELECT DISTINCT ProjectTags.'+self.PROJECTTAGITEMS[0]
        for item in self.PROJECTTAGITEMS[1:]:
            com = com + ', ProjectTags.'+item
    ...
    def getJobEtree(self, jobId=None, jobInfo={}):
        ...
        if len(inputFileIds) > 0:
            ...
            for fileId in inputFileIds:
                try:
                    fileItem, err = self.getFileEtree(
                        fileId=fileId, role=FILE_ROLE_IN, projectId=projectId)
                    errReport.extend(err)
                    fileBranch.append(fileItem)
                except:
    ...
    def exportProjectXml(self, projectId=None, fileName=None, recordExport=False, after=None, status=None, jobList=None, inputFileList=None, inputFileFromJobList=None):
        ...
        try:
            ...
            f.header.projectName.set(
                self.getProjectInfo(projectId, 'projectname'))
        except:
            errReport.append(self.__class__, 257, fileName)
        # tree,err = self.getProjectEtree(projectId)
        tree, jobNumberList, err = self.getTablesEtree(
            projectId, after=after, status=status, jobList=jobList, inputFileList=inputFileList, inputFileFromJobList=inputFileFromJobList)
        # print 'CDbApi.exportProjectXml',tree,err
        errReport.extend(err)
        if recordExport:
            exportId = self.createProjectExport(
                projectId=projectId, projectExportAfter=after)
            table = self.getProjectExportInfo(
                projectExportId=exportId, returnList=True)
            exportTableEle = self.tableEtree(
                'projectexport', ('projectexportid', 'projectexporttime', 'projectexportafter'), table)
    ...
    def updateProject(self, projectId=None, key=None, value=None):
        ...
        if not self._userRole in [USER_ROLE_MANAGER, USER_ROLE_OWNER]:
            raise CException(self.__class__, 106, 'You are ' +
                             self._userName+' calling updateProject')
        ...
        # self.setDiagnostic(True)
        self.execute("UPDATE Projects SET "+key +
    ...
    def deleteProject(self, projectId=None, deleteChildren=False):
       ...
       # Delete all jobs/files/comments/XData in the project
        ...
        self.execute(
            'DELETE FROM FileUses WHERE JobId IN (SELECT JobId FROM Jobs WHERE Jobs.ProjectId = ?)', (projectId,))
        self.execute(
            'DELETE FROM Files WHERE JobId IN (SELECT JobId FROM Jobs WHERE Jobs.ProjectId = ?)', (projectId,))
        self.execute(
            'DELETE FROM XData WHERE JobId IN (SELECT JobId FROM Jobs WHERE Jobs.ProjectId = ?)', (projectId,))
        self.execute(
            'DELETE FROM Comments WHERE JobId IN (SELECT JobId FROM Jobs WHERE Jobs.ProjectId = ?)', (projectId,))
        self.execute(
            'DELETE FROM JobKeyValues WHERE JobId IN (SELECT JobId FROM Jobs WHERE Jobs.ProjectId = ?)', (projectId,))
        self.execute(
            'DELETE FROM JobKeyCharValues WHERE JobId IN (SELECT JobId FROM Jobs WHERE Jobs.ProjectId = ?)', (projectId,))
        self.execute(
            'DELETE FROM ProjectImports WHERE ProjectId = ?', (projectId,))
        self.execute(
            'DELETE FROM ProjectExports WHERE ProjectId = ?', (projectId,))
        self.execute(
            'DELETE FROM ProjectTags WHERE ProjectId = ?', (projectId,))
        self.execute(
            'DELETE FROM ProjectComments WHERE ProjectId = ?', (projectId,))

        # Delete Jobs and ProjectsUsersPermissions
        print('Start deleting jobs etc')
        self.execute(
            "UPDATE Projects SET FollowFromJobId=NULL WHERE ProjectId = ?", (projectId,))
        self.execute(
            "UPDATE Projects SET ParentProjectId=NULL WHERE ParentProjectId = ?", (projectId,))
        self.execute("DELETE FROM Jobs WHERE ProjectId = ?", (projectId,))
        self.execute(
            "DELETE FROM ProjectsUsersPermissions WHERE ProjectID = ?", (projectId,))

        # Delete from Projects table too
        # print 'CDbApi.deleteProject',projectId,type(projectId)
        print('Start deleting project')
        self.execute("DELETE FROM Projects WHERE ProjectID = ?",
                     (str(projectId),))
    ...
    def getAllInfo(self, toTerm=False):
        ...
        for project in projects:
            ...
            pdbs = self.getJobsWithOutputFiles(
                projectId=projectId, fileType='chemical/x-pdb')
            mtzs = self.getJobsWithOutputFiles(
                projectId=projectId, fileType='application/CCP4-mtz-map', subType=1)
            mtzdiffs = self.getJobsWithOutputFiles(
                projectId=projectId, fileType='application/CCP4-mtz-map', subType=2)
    ...
    def listProjectNames(self, toTerm=False):
        self.execute(
            "SELECT  ProjectName  FROM Projects ORDER BY ProjectCreated")
    ...
    def listProjects(self, toTerm=False, order='date'):
        try:
            if order == 'date':
                self.execute(
                    "SELECT ProjectID, ProjectName, ProjectDirectory, ParentProjectID  FROM Projects ORDER BY ProjectCreated")
            elif order == 'name':
                self.execute(
                    "SELECT ProjectID, ProjectName, ProjectDirectory, ParentProjectID  FROM Projects ORDER BY ProjectName")
            elif order == 'access':
                self.execute(
                    "SELECT ProjectID, ProjectName, ProjectDirectory, ParentProjectID  FROM Projects ORDER BY LastAccess DESC")
            self._projectList = self.fetchAll2PyList(
                [UUIDTYPE, str, str, UUIDTYPE])
        except:
            pass
        if toTerm:
            print('listProjects')
            for item in self._projectList:
                print(item)
    ...
    def nextJobNumber(self, projectId, childOf=None, reserve=1):
        if childOf is None:
            ...
            self.execute(
                "SELECT LastJobNumber FROM Projects WHERE ProjectID = ? ", (projectId,))
            ...
            if reserve > 0:
                ...
                self.execute(
                    "UPDATE Projects SET LastJobNumber = ? WHERE ProjectID = ? ", (jobNo, projectId))
                self.commit()
            ...
            self.execute(
                "SELECT jobNumber from Jobs WHERE ProjectID = ? and JobID = ?", (projectId, childOf))
            jobNoList = self.fetchAll2Py(str)
            if len(jobNoList) != 1:
                raise CException(self.__class__, 123, str(projectId))
            jobNoStr = jobNoList[0]
            self.execute(
                "SELECT jobNumber from Jobs WHERE ProjectID = ? and ParentJobID = ?", (projectId, childOf))
            jobNoList = self.fetchAll2Py(str)
            maxNo = 0
    ...
    def getTaskNameLookup(self, projectId=None, jobId=None, extras=False):
        if extras:
            ...
            SELECT Jobs.JobNumber,Jobs.TaskName,Jobs.JobId,Jobs.Status,Jobs.FinishTime FROM Jobs WHERE Jobs.JobId IN subJobs;""")
                ...
                self.execute(
                    'SELECT JobNumber,TaskName,JobId,Status,FinishTime FROM Jobs WHERE ProjectID = ?', (projectId,))
            rv = self.fetchAll2PyList([str, str, UUIDTYPE, int, float])
        else:
            self.execute(
                'SELECT JobNumber,TaskName FROM Jobs WHERE ProjectID = ?', (projectId,))
            rv = self.fetchAll2PyList([str, str])
        return rv
    ...
    def getJobId(self, projectName=None, jobNumber=None, projectId=None):
        ...
        if jobNumber is None:
            raise CException(self.__class__, 134, str(
                projectName)+':'+str(jobNumber))
        u = (projectId, jobNumber,)
        self.execute(
            'SELECT JobID FROM Jobs WHERE ProjectID = ? AND JobNumber = ?', u)
        rv = self.fetchAll2Py(UUIDTYPE)
        # print('CDbApi.getJobId',projectId,jobNumber,rv)
        if len(rv) == 0:
            raise CException(self.__class__, 134, str(
                projectId)+':'+str(jobNumber))
        else:
            return rv[0]
    ...
    def getJobsInRange(self, projectId=None, jobIdRange=None, jobNumberRange=None):
        ...
        if jobIdRange is not None:
            ...
            self.execute(
                'SELECT CreationTime FROM Jobs WHERE ProjectId =? AND JobID IN (?,?)', args)
        elif jobNumberRange is not None:
            args.extend(jobNumberRange)
            self.execute(
                'SELECT CreationTime FROM Jobs WHERE  ProjectId =? AND JobNumber IN (?,?)', args)
        ...
        if timeRange[0] < timeRange[1]:
            self.execute(
                'SELECT JobId FROM Jobs WHERE CreationTime >= ? AND CreationTime <= ? AND ParentJobId IS NULL', timeRange)
        else:
            self.execute(
                'SELECT JobId FROM Jobs WHERE CreationTime <= ? AND CreationTime >= ?  AND ParentJobId IS NULL', timeRange)
        rv = self.fetchAll2Py(UUIDTYPE)
    ...
    def createJob(self, projectId, taskName, parentJobId=None, jobTitle=None, status=JOB_STATUS_PENDING, jobNumber=None, taskVersion=None, userAgent=None, userName=None):
        ...
        if jobId is not None:
            ...
            args = [projectId, parentJobId, jobNumber, userAgent,
                    taskName, jobTitle, time, JOB_STATUS_PENDING, userId, jobId]
            ...
            args = [jobId, projectId, parentJobId, jobNumber, userAgent,
                    taskName, jobTitle, time, JOB_STATUS_PENDING, userId]
        ...
        self.jobCreated.emit(
            {'jobId': jobId, 'projectId': projectId, 'parentJobId': parentJobId})
    ...
    def updateJobStatus(self, jobId=None, status=None, jobNumber=None, projectName=None):
        ...
        self.execute("UPDATE Jobs SET status = ? WHERE JobID = ?",
                     (status, jobId,))
        ...
        if status in (JOB_STATUS_FINISHED, JOB_STATUS_FAILED, JOB_STATUS_INTERRUPTED, JOB_STATUS_TO_DELETE, JOB_STATUS_UNSATISFACTORY):
            self.execute(
                "UPDATE Jobs SET FinishTime = ? WHERE JobID = ?", (self.currentTime(), jobId,))
            # print 'CDbApi.updateJobStatus finished',jobId,status
            self.commit()
            self.jobFinished.emit(
                {'jobId': jobId, 'projectId': projectId, 'status': status})
        else:
            self.commit()

        self.jobUpdated.emit(
            {'jobId': jobId, 'projectId': projectId, 'key': 'status', 'value': status})
        self.jobStatusUpdated.emit(
            {'jobId': jobId, 'status': JOB_STATUS_TEXT[status]})

    def updateJob(self, jobId=None, key=None, value=None):

        if key is None:
            raise CException(self.__class__, 169, 'None')
        key = key.lower()
        ...
        if key == 'preceedingjobid':
            ...
            if value is None or len(value) == 0:
                ...
                self.execute(
                    'SELECT ProjectId from Jobs WHERE JobId = ?', (UUIDTYPE(value),))
            ...
            if isinstance(value, int) and value >= 0 and value < len(JOB_EVALUATION_TEXT):
                ...
                raise CException(self.__class__, 190, 'Key: ' +
                                 str(key)+'  Value: '+str(value))
        if value is None or value == 0 or value == '':
            self.execute("UPDATE Jobs SET "+key +
        ...
        self.jobUpdated.emit(
            {'jobId': jobId, 'projectId': projectId, 'key': key, 'value': value})
    ...
    def deleteJob(self, jobId=None, projectName=None, jobNumber=None, deleteChildren=False):
        ...
        self.jobDeleted.emit(
            {'jobId': jobId, 'projectId': jobInfo['projectid'], 'jobNumber': jobInfo['jobnumber'], 'deleteChildren': deleteChildren})
    ...
    def _deleteOneJob(self, jobId, saveJob=False, saveImportFiles=False):
        ...
        # self.setDiagnostic(True)
        self.execute(
            "UPDATE Jobs SET PreceedingJobId = ? WHERE PreceedingJobId = ?", (None, jobId))
        self.execute(
            "UPDATE Projects SET FollowFromJobId = ? WHERE FollowFromJobId = ?", (None, jobId))
        ...
        if not saveImportFiles:
            self.execute(
                "DELETE FROM ImportFiles WHERE FileId IN (SELECT FileId FROM Files WHERE Files.JobId= ? )", (jobId,))
        self.execute(
            "DELETE FROM ExportFiles WHERE FileId IN (SELECT FileId FROM Files WHERE Files.JobId= ? )", (jobId,))
        self.execute(
            "DELETE FROM FileUses WHERE FileId IN (SELECT FileId FROM Files WHERE Files.JobId= ? )", (jobId,))
        ...
        self.execute(
            "DELETE FROM FileAssociationMembers WHERE FileId IN (SELECT FileId FROM Files WHERE Files.JobId= ? )", (jobId,))
    ...
    def getJobAncestors(self, jobId=None):
        ...
        while 1:
            self.execute(
                'SELECT parentjobid FROM Jobs WHERE jobid=?', (jobPath[0],))
            rv = self.fetchAll2Py(UUIDTYPE)
    ...
    def getJobInfo(self, jobId=None, mode='all', projectName=None, jobNumber=None, returnType=None):
        ...
        for item in itemList:
            if item == 'projectname':
                if not itemList.count('projectid'):
                    retList.append('projectid')
                xtrasList.append('projectname')
            elif item == 'runtime':
                if not itemList.count('creationtime'):
                    retList.append('creationtime')
                if not itemList.count('finishtime'):
                    retList.append('finishtime')
                xtrasList.append('runtime')
            elif item == 'parentjobnumber':
                if not itemList.count('parentjobid'):
                    retList.append('parentjobid')
                xtrasList.append('parentjobnumber')
            elif item in ['childjobs', 'descendentjobs']:
                xtrasList.append(item)
            elif item in ['performanceclass']:
                if not itemList.count('taskname'):
                    retList.append('taskname')
                xtrasList.append(item)
        ...
        if xtrasList.count('childjobs'):
            ret['childjobs'] = self.getChildJobs(jobId)
        if xtrasList.count('descendentjobs'):
            ret['descendentjobs'] = self.getChildJobs(jobId, descendents=True)
        if xtrasList.count('performanceclass'):
            ret['performanceclass'] = self.getJobPerformanceClass(
                jobId, taskName=ret.get('taskname'))
    ...
    def getJobPerformanceClass(self, jobId=None, taskName=None):
        ...
        selcom = 'SELECT XDataClass,XDataXml FROM XData WHERE JobId=? AND XDataClass in ' + \
            CCP4PerformanceData.performanceIndicatorClasses()
        self.execute(selcom, (jobId,))
        rv = self.fetchAll2PyList([str, str])
        if len(rv) > 0:
            try:
    ...
    def getJobPerformance(self, jobId=None):
        self.execute(
            'SELECT KeyTypeId,Value FROM JobKeyValues WHERE jobId = ?', (jobId,))
        rv = self.fetchAll2PyList([int, float])
        self.execute(
            'SELECT KeyTypeId,Value FROM JobKeyCharValues WHERE jobId = ?', (jobId,))
    ...
    def getJobFilesInfo(self, jobId=None, jobParamName=None, fileTypeId=None, input=False):
        ...
        rv = self.fetchAll2PyList(
            [str, str, str, str, int, int, int, str, str])
        ret = []
        ...
        for item in rv:
            ...
            ret[-1].update(self.getJobInfo(ret[-1]['jobId'],
                           ['jobnumber', 'taskname']))
    ...
    def getProjectJobListInfo(self, mode='all', projectId=None, projectName=None, topLevelOnly=False, order=None,
        ...
        if ('childjobs' in mode or order is not None or
                firstJob is not None or lastJob is not None) and not 'jobid' in itemList:
            itemList.append('jobid')
        if 'projectname' in itemList and not 'projectid' in itemList:
            itemList.append('projectid')
        if ('performance' in itemList or 'performanceclass' in itemList) and not 'jobid' in itemList:
            itemList.append('jobid')
        if 'performanceclass' in itemList and not 'taskname' in itemList:
            itemList.append('taskname')
        ...
        rv = self.fetchAll2PyList(typeConv)
        # print 'getProjectJobListInfo rv',rv
        if maxJobs is not None and len(rv) > maxJobs:
            rv = rv[0:maxJobs]
      ...
      if 'performanceclass' in itemList:
        ...
        if 'performance' in itemList or 'performanceclass' in itemList:
            self.execute(
                'SELECT JobId,KeyTypeId,Value FROM JobKeyValues WHERE JobId IN (SELECT JobId FROM Jobs WHERE ProjectId=?  )', (projectId,))
            perfList = self.fetchAll2PyList([UUIDTYPE, int, float])
            self.execute(
                'SELECT JobId,KeyTypeId,Value FROM JobKeyCharValues WHERE JobId IN (SELECT JobId FROM Jobs WHERE ProjectId=? )', (projectId,))
            perfList.extend(self.fetchAll2PyList([UUIDTYPE, int, str]))

            for jid, keyId, value in perfList:
                try:
                    if jid not in perfDict:
                        perfDict[jid] = {}
                    perfDict[jid][KEYTYPELIST[keyId][1]] = value
                except:
                    print(
                        'Error in CDbApi.getProjectJobListInfo creating Performace Indicator data object')
            # print 'getProjectJobListInfo.perfDict', perfList, perfDict

        retList = []
        for jobValue in rv:
            retList.append(self.jobInfoDict(
                itemList, jobValue, performanceDict=perfDict))
    ...
    def jobInfoDict(self, itemList, jobValue, extras=[], performanceDict={}):
        ...
        for item in itemList:
            ...
            if item == 'status':
                ...
                ret[item] = performanceDict.get(
                    jobValue[itemList.index('jobid')], None)
            elif item in ['performanceclass']:
                ret[item] = self.getJobPerformanceClass(
                    jobValue[itemList.index('jobid')], jobValue[itemList.index('taskname')])
                cls = CCP4TaskManager.TASKMANAGER().getPerformanceClass(
                    jobValue[itemList.index('taskname')])
                if cls is None:
                    ret[item] = None
                else:
                    try:
                        ...
                        print(
                            'Error in CDbApi.jobInfoDict creating Performance data object', e)
                        ret[item] = None
        ...
        for item in extras:
            if item == 'runtime':
                ...
                if finishtime is not None:
                    ret[item] = finishtime - \
                        jobValue[itemList.index('creationtime')]
                else:
                    ret[item] = 0.0
            elif item == 'projectname':
                ret[item] = self.getProjectInfo(
                    projectId=jobValue[itemList.index('projectid')], mode='projectname')
            elif item == 'parentjobnumber':
                if jobValue[itemList.index('parentjobid')] is not None:
                    ret[item] = self.getJobInfo(
                        jobId=jobValue[itemList.index('parentjobid')], mode='jobnumber')
                else:
                    ret[item] = None
    ...
    def getChildJobs(self, jobId, descendents=False, details=False):
        self.execute(
            "SELECT JobId FROM Jobs WHERE ParentJobId = ? ORDER BY CreationTime", (jobId,))
        ...
        if not descendents:
            if details:
                ...
                for child in childList:
                    ret = self.getJobInfo(
                        jobId=child, mode=['taskname', 'jobnumber'])
                    detailsList.append(
                        [ret['jobnumber'], child, ret['taskname']])
                detailsList.sort()
                return detailsList
            else:
                return childList
    ...
    def getFileTypeId(self, fileTypeName=None, mimeType=None):
        if fileTypeName is not None:
            u = (fileTypeName,)
            self.execute(
                'SELECT FileTypeID FROM FileTypes WHERE FileTypeName = ?', u)
            rv = self.fetchAll2Py(int)
            if len(rv) == 0:
    ...
    def getParamsContainer(self, jobId):
        ...
        if not os.path.exists(paramsFile):
            raise CException(self.__class__, 170, paramsFile)
    ...
    def deleteFileUses(self, jobId=None):
        self.execute(
            "DELETE FROM FileUses WHERE JobId=? AND RoleId=?", (jobId, FILE_ROLE_IN))
        self.commit()
    ...
    def gleanJobFiles(self, jobId=None, container=None, projectId=None, dbOutputData=None, roleList=[FILE_ROLE_IN, FILE_ROLE_OUT], unSetMissingFiles=False):
        ...
        projectName = self.getProjectInfo(
            projectId=projectId, mode='projectname')
        ...
        if container.header.pluginVersion.isSet():
            self.updateJob(jobId=jobId, key='taskversion',
                           value=container.header.pluginVersion.__str__())

        for file_role in roleList:
            subcontainer = (container.inputData, container.outputData)[
                [FILE_ROLE_IN, FILE_ROLE_OUT].index(file_role)]
            ...
            for key in keyList:
                ...
                for obj in objList:
                    ...
                    if isinstance(obj, CCP4File.CDataFile) and obj.isSet():
                        idx += 1
                        if not obj.exists():
                            errorReport.append(self.__class__, 173, str(
                                obj.objectName())+' '+str(obj), stack=False)
                            if unSetMissingFiles:
                                obj.unSet()
                        else:
                            if len(objList) > 1:
                                importList.append(
                                    [obj, jobParamName+'.'+str(idx)])
                            else:
                        ...
                        for fileObj in obj:
                            ...
                            if fileObj.exists():
                                importList.append(
                                    [fileObj, jobParamName+'.'+str(idx)])
                            elif fileObj.isSet():
                                errorReport.append(self.__class__, 173, str(
                                    fileObj.objectName())+' '+str(fileObj), stack=False)
                                if unSetMissingFiles:
                                    fileObj.unSet()
                ...
                for obj, jobParamName in importList:
                    ...
                    if fileId is not None:
                        print('Recording file in database matches existing',
                              key, str(obj), fileId, fromJobId)
                    if fileId is None:
                        if file_role == FILE_ROLE_OUT:
                            try:
                                # print 'gleanJobFiles output file',obj.objectName()
                                fileId = self.createFile(
                                    jobId=jobId, fileObject=obj, projectId=projectId, jobParamName=jobParamName)
                                obj.dbFileId = fileId
                                ...
                                errorReport.append(
                                    self.__class__, 171, str(e), stack=False)
                        else:
                            print(
                                'ERROR in gleanJobFiles - no fileId for an input file', obj)
                            try:
                                fileId = self.createFile(
                                    jobId=None, fileObject=obj, projectId=projectId, jobParamName=jobParamName)
                                obj.dbFileId = fileId
                                # print 'gleanJobFile setting dbFileId',obj.objectName(),obj,fileId
                            except CException as e:
                                errorReport.extend(e, stack=False)
                            ...
                            try:
                                self.createFileUse(
                                    jobId=jobId, fileId=fileId, role=file_role, jobParamName=jobParamName)
                            except CException as e:
                                errorReport.extend(e, stack=False)
                            except Exception as e:
                                errorReport.append(
                                    self.__class__, 172, str(e), stack=False)
                    else:
                        if file_role == FILE_ROLE_OUT:
                  ...
                  '''
                            ...
                            if fromJobId is not None:
                                if not (fileType, fileId, fromJobId) in preceedingJobs:
                                    preceedingJobs.append(
                                        [fileType, fileId, fromJobId])
                            try:
                                self.createFileUse(
                                    jobId=jobId, fileId=fileId, role=file_role, jobParamName=jobParamName)
                                if not obj.dbFileId.isSet():
                                    obj.dbFileId = fileId
                                print('Recording file in database',
                                      jobParamName, obj, fileId)
                            except CException as e:
                                errorReport.extend(e, stack=False)
                            except Exception as e:
                                errorReport.append(
                ...
                if file_role == FILE_ROLE_OUT and xmlText is not None:
                    ...
                    self.createXData(
                        jobId=jobId, dataClass=dataClass, dataXml=xmlText)
                if file_role == FILE_ROLE_OUT and len(keyValues) > 0:
                    ...
                    for keyTypeName, value in list(keyValues.items()):
                        self.createJobKeyValue(
                            jobId=jobId, keyTypeName=keyTypeName, value=value)
    ...
    def _makeJobFileName(self, jobId=None, mode='PARAMS'):
        ...
        import os
        path = os.path.join(self.jobDirectory(jobId=jobId),
                            defFiles.get(mode, 'unknown.unk'))
    ...
    def createImportFile(self, fileId=None, sourceFileName=None, sourceFileId=None, jobId=None, annotation=None,
        ...
        if checksum is None and sourceFileName is not None and os.path.exists(sourceFileName):
            try:
                ...
                print('CDbApi.createImportFile calculated checksum',
                      sourceFileName, checksum)
            except:
                print('Failed attempting to calculate checksum', sourceFileName)

        args = (importId, fileId, time, lastModifiedTime, checksum,
                sourceFileName, sourceFileId, annotation, importNumber, reference)
    ...
    def updateImportFile(self, importId=None, key=None, value=None, jobId=None):
        if key is None:
            raise CException(self.__class__, 252)
        key = key.lower()
        if not key in ['annotation', 'exportfileid', 'creationtime']:
            raise CException(self.__class__, 252, key)

        # self.setDiagnostic(True)
        self.execute("UPDATE ImportFiles SET "+key +
                     " = ? WHERE ImportID = ?", (value, importId,))
        # self.setDiagnostic(False)
        self.commit()

        if jobId is None:
            self.execute(
                'SELECT Files.JobId FROM Files INNER JOIN ImportFiles ON Files.FileId = ImportFiles.FileId WHERE ImportFiles.ImportId=?', (importId,))
            jobId = self.fetchAll2Py(UUIDTYPE)[0]

        self.importFileUpdated.emit(
            {'jobId': jobId, 'importId': importId, 'key': key, 'value': value})
    ...
    def getImportFileInfo(self, importId=None, mode='all', fileId=None):
        ...
        if mode == 'all':
            ...
            for item in mode:
                if item.lower() in self.IMPORTFILEITEMS:
                    itemList.append(item.lower())
                if item.lower() in ['fileid', 'jobid']:
                    fileList.append(item.lower())
        ...
        if len(itemList) > 0:
            for item in itemList:
                selcom = selcom + item + ', '
                typeConv.append(
                    self.IMPORTFILETYPES[self.IMPORTFILEITEMS.index(item)])
            ...
            # print 'getProjectJobListInfo rv',rv
            if len(rv) != 1:
                raise CException(self.__class__, 271)
            i = 0
            for item in itemList:
                ret[item] = rv[0][i]
                i = i + 1
    ...
    def getImportFileInstances(self, jobId=None, brief=True):
        if brief:
            ...
            rv = self.fetchAll2PyList(
                [UUIDTYPE, float, float, str, UUIDTYPE, str])
            retList = []
            for importid, time, lastmodifiedtime, sourcefilename, fileid, annotation in rv:
                retList.append({'importid': importid, 'sourcefilename': sourcefilename, 'creationtime': time,
                               'lastmodifiedtime': lastmodifiedtime, 'fileid': fileid, 'annotation': annotation})
            return retList
        else:
            ...
            for f in rv:
                ...
                for i in range(len(self.FILEITEMS)):
                    if self.FILEITEMS[i] not in ['jobid']:
                        infoList[-1][self.FILEITEMS[i]
                                     ] = f[i+len(self.IMPORTFILEITEMS)]
                infoList[-1]['filetypename'] = FILETYPES_TEXT[infoList[-1]
                                                              ['filetypeid']][1]
    ...
    def createExportFile(self, fileId=None, exportFilename=None):
        ...
        self.execute(
            "INSERT INTO ExportFiles (ExportID,CreationTime,ExportFilename,FileId) VALUES (?,?,?,?)", args)
        self.commit()
    ...
    def getExportFileInstances(self, fileId=None, jobId=None):
        ...
        if fileId is not None:
            self.execute(
                'SELECT ExportID, CreationTime, ExportFilename, FileId FROM ExportFiles WHERE FileId = ?', (fileId,))
        ...
        for exportid, time, exportfilename, fileid in rv:
            retList.append({'exportid': exportid, 'exportfilename': exportfilename,
                           'creationtime': time, 'fileid': fileid})
        return retList
    ...
    def getExportFileInfo(self, exportId=None, mode='all'):
        ...
        for exportid, creationtime, exportfilename, jobid, jobnumber, taskname in rv:
            retList.append({'exportid': exportid, 'exportfilename': exportfilename,
                           'creationtime': creationtime, 'jobid': jobid, 'jobnumber': jobnumber, 'taskname': taskname})
        if len(retList) == 0:
            return {}
        else:
            return retList[0]
    ...
    def getExportFilesByFileType(self, fileTypeClass=None, before=None, projectId=None):
        ...
        for exportid, creationtime, exportfilename, jobid, jobnumber, taskname in rv:
            retList.append({'exportid': exportid, 'exportfilename': exportfilename,
                           'creationtime': time, 'jobid': jobid, 'jobnumber': jobnumber, 'taskname': taskname})
        return retList
    ...
    def createFile(self, jobId=None, projectId=None, fileObject=None, fileTypeId=None, fileTypeName=None,
        ...
        if self._preferences['testFileExists']:
            if fileObject is not None and not fileObject.exists():
                raise CException(self.__class__, 162,
                                 fileObject.fullPath.__str__())
            ...
            if sys.platform[0:3] != 'win' and fileObject is not None and sourceFileName is None and sourceFileId is None:
                ...
                try:
                    ...
                    raise CException(self.__class__, 128, fileObject.fullPath.__str__(
                    )+' '+self.jobDirectory(jobId=jobId), stack=False)
                if not isSame:
                    raise CException(self.__class__, 127, fileObject.fullPath.__str__(
                    )+' expected in '+str(self.jobDirectory(jobId=jobId)), stack=False)
            else:
                pass
        ...
        if fileObject is not None:
            ...
            if jobParamName is None or len(jobParamName) == 0:
        ...
        if fileTypeId is None:
            if fileTypeName is None:
                fileTypeName = fileObject.qualifiers('mimeTypeName')
        ...
        args = [fileId, jobId, jobParamName, baseName, fileTypeId,
                annotation, fileContent, subType, filePathFlag]
        ...
        self.fileCreated.emit(
            {'fileId': fileId, 'jobId': jobId, 'projectId': projectId})
        return fileId
    ...
    def createFileUse(self, jobId=None, fileId=None, role=FILE_ROLE_IN, jobParamName=None):
        ...
        self.execute(
            "INSERT INTO FileUses (FileID,JobID,RoleID,JobParamName) VALUES (?,?,?,?)", args)
        self.commit()
        self.fileUseCreated.emit(
            {'fileId': fileId, 'jobId': jobId, 'projectId': projectId, 'role': role})

    def matchFileName(self, fileName=None, fileObject=None):
        if fileObject is not None:
            if fileObject.dbFileId.isSet():
                try:
                    jobId = self.getFileInfo(
                        fileId=fileObject.dbFileId.get(), mode='jobid')
                except:
                    pass
                else:
            ...
            if fileObject.project.isSet():
                try:
                    fileProjectId = self.getProjectId(
                        projectName=str(fileObject.project))
                except CException as e:
                    # Not recognised as project - is it an alias?
                    try:
                        aliasPath = self.getAliasDirectory(
                            str(fileObject.project))
                    except:
                        return None, None
            ...
            fileProjectId, fileProjectName, fileProjectPath = self.matchProjectDirectory(
                path)
        ...
        if fileProjectId is not None:
            ...
            if len(jobNumber) > 0:
                ...
                self.execute(
                    'SELECT  Files.FileId,Files.JobId FROM Files INNER JOIN Jobs ON Files.JobId = Jobs.JobId WHERE Files.FileName= ? AND Jobs.JobNumber = ? AND Jobs.ProjectId = ?', args)
                ...
                self.execute(
                    'SELECT Files.FileId,Files.JobId FROM Files INNER JOIN Jobs ON Files.JobId=Jobs.JobId WHERE Files.Filename = ?  AND Jobs.ProjectID = ?', args)
                fileIdList = self.fetchAll2PyList([UUIDTYPE, UUIDTYPE])
    ...
    def getFileInfo(self, fileId=None, mode='all'):
        ...
        if 'sourcefilename' in itemList:
            if not 'importid' in itemList:
                itemList.append('importid')
        if 'relpath' in itemList:
            if not 'jobnumber' in jobItems:
                jobItems.append('jobnumber')
            if not 'jobid' in itemList:
                itemList.append('jobid')
            if not 'importid' in itemList:
                itemList.append('importid')
            if not 'filetypeid' in itemList:
                itemList.append('filetypeid')
            if not 'pathflag' in itemList:
                itemList.append('pathflag')
            ifRelPath = True
        else:
            ifRelPath = False
        for item in itemList:
        ...
        if 'importid' in retList:
            selcom = selcom[0:-2] + \
                " FROM Files LEFT OUTER JOIN ImportFiles ON Files.FileId=ImportFiles.FileId WHERE Files.FileID=?"
        else:
        ...
        if len(rv) != 1:
            ...
            if len(jobItems) > 0:
                if ret['jobid'] is None:
                    jobInfo = {'jobnumber': None, 'taskname': None,
                               'projectid': None, 'projectname': None, 'relpath': None}
                else:
                    jobInfo = self.getJobInfo(
                        jobId=ret['jobid'], mode=jobItems, returnType=dict)
                    ...
                    if ifRelPath:
                        ret['relpath'] = self.getRelPath(
                            ret['pathflag'], ret['filetypeid'], jobInfo['jobnumber'], ret['importid'])
    ...
    def getFullPath(self, fileId=None, projectDirectory=None):
        ...
        if projectDirectory is None or importId is None:
            if jobId is None:
                raise CException(self.__class__, 168)
            self.execute(
                'SELECT JobNumber,ProjectId FROM Jobs WHERE JobId = ?', (jobId,))
            jobList = self.fetchAll2PyList([str, UUIDTYPE])
            if len(jobList) != 1:
                raise CException(self.__class__, 168)
            jobNumber, projectId = jobList[0]
            if projectDirectory is None:
                projectDirectory = self.getProjectDirectory(projectId)
    ...
    def getJobImportFiles(self, jobId=None):
        ...
        fileList = self.fetchAll2PyList(
            [UUIDTYPE, UUIDTYPE, UUIDTYPE, int, str, str])
        return fileList
    ...
    def getJobFiles(self, jobId=None, role=FILE_ROLE_OUT, mode='fileId', searchFileUses=True, fileTypes=None):
        ...
        if role == FILE_ROLE_OUT:
            ...
            if mode == 'all':
                self.execute(
                    'SELECT Files.JobId,Files.FileID,ImportFiles.ImportId,Files.FileTypeId,Files.Filename,Files.Annotation FROM Files'+seleList, args)
                outputList = self.fetchAll2PyList(
                    [UUIDTYPE, UUIDTYPE, UUIDTYPE, int, str, str])
            elif mode == 'fileId':
                ...
                self.execute(
                    'SELECT Files.Annotation FROM Files'+seleList, args)
                outputList = self.fetchAll2Py(str)
            elif mode in ['fileType', 'fileTypeId']:
                self.execute(
                    'SELECT Files.FileTypeID FROM Files'+seleList, args)
        ...
        if mode == 'all':
            ...
            fileUseList = self.fetchAll2PyList(
                [UUIDTYPE, UUIDTYPE, int, str, str])
            for fileList in fileUseList:
                outputList.append(
                    [fileList[0], fileList[1], None, fileList[2], fileList[3], fileList[4]])
        elif mode == 'fileId':
            self.execute(
                'SELECT FileID FROM FileUses WHERE JobID = ? AND RoleID = ?', args)
            rv = self.fetchAll2Py(UUIDTYPE)
            ...
            for jobId, fileName in fileList:
                if jobId is not None:
                    ...
                    outputList.append(os.path.join(
                        diry, 'CCP4_IMPORTED_FILES', fileName))
        elif mode == 'fileName':
            self.execute(
                'SELECT Files.Filename FROM Files INNER JOIN FileUses ON Files.FileId=FileUses.FileId WHERE FileUses.JobID = ? AND FileUses.RoleID = ?'+fileTypeSele, args)
            outputList.extend(self.fetchAll2Py(str))
        elif mode == 'annotation':
            self.execute(
                'SELECT Files.Annotation FROM Files INNER JOIN FileUses ON Files.FileId=FileUses.FileId WHERE FileUses.JobID = ? AND FileUses.RoleID = ?'+fileTypeSele, args)
            outputList.extend(self.fetchAll2Py(str))
        elif mode == 'fileType':
            self.execute(
                'SELECT Files.FileTypeID FROM Files INNER JOIN FileUses ON Files.FileId=FileUses.FileId WHERE FileUses.JobID = ? AND FileUses.RoleID = ?'+fileTypeSele, args)
            typesList = self.fetchAll2Py(int)
    ...
    def getProjectFiles(self, projectId=None, fileType=None, subType=None, contentFlag=None, topLevelOnly=True):
        ...
        if subType is not None or contentFlag is not None:
            com, args = self.fileTypeCommand(
                com, args, subType=subType, contentFlag=contentFlag)
        self.execute(com, args)
        fileList = self.fetchAll2PyList(
            [UUIDTYPE, UUIDTYPE, UUIDTYPE, int, str, str, str])
        return fileList
    ...
    def deleteFilesOnJobIdAndParamName(self, jobIdParamList=[]):
        ...
        for ii in range(len(jobIdParamList)):
            ...
            try:
                self.execute(
                    com, (jobIdParamList[ii][0], jobIdParamList[ii][1]+'%'))
            except:
                print('Failed delete in deleteFilesOnJobIdAndParamName',
                      jobIdParamList[ii])
        self.setDiagnostic(False)
        self.commit()
    ...
    def deleteFilesOnJobNumberAndParamName(self, projectId=None, jobNumberParamList=[]):
        ...
        for ii in range(len(jobNumberParamList)):
            ...
            try:
                self.execute(
                    com, (projectId, jobNumberParamList[ii][0], jobNumberParamList[ii][1]+'%'))
            except:
                print('Failed delete in deleteFilesOnJobNumberAndParamName',
                      jobNumberParamList[ii])
        self.commit()
    ...
    def getMultiFollowOnJobs(self, jobIdList, traceImportFiles=True):
        ...
        for jobId in jobIdList:
            jobTreeList.append(self.getFollowOnJobs(
                jobId, traceImportFiles=traceImportFiles))
        return jobTreeList
    ...
    def getPreceedingJobs(self, jobId=None, fileType=None):
        ...
        if fileType is None:
            ...
            for ii in range(len(fileType)-2):
                com = com + ',?'
            com = com + ')'
    ...
    def getFollowOnJobs(self, jobId=None, traceImportFiles=True, fileId=None):
        ...
        if fileId is not None:
            args = (fileId,)
            self.execute(
                'SELECT DISTINCT JobID FROM FileUses WHERE FileId = ?')
            childJobList = self.fetchAll2Py(UUIDTYPE)
            args = (fileId,)
            self.execute(
                'SELECT ImportFiles.ImportId,Files.Filename FROM ImportFiles INNER JOIN Files ON ImportFiles.FileId=Files.FileId WHERE =Files.FileId= ?', args)
    ...
    def setJobToImport(self, jobId=None, projectId=None, importFiles=[]):
        ...
        if projectId is None:
            projectId = self.getJobInfo(jobId, 'projectid')
        self.setJobToImportSignal.emit(
    ...
    def getProjectImportFiles(self, projectId=None, ifJobInfo=False):
        ...
        if ifJobInfo:
            ...
            fileList = self.fetchAll2PyList(
                [UUIDTYPE, UUIDTYPE, UUIDTYPE, int, str, str, str, str, str, float])
        else:
            ...
            fileList = self.fetchAll2PyList(
                [UUIDTYPE, UUIDTYPE, UUIDTYPE, int, str, str, str, float])
    ...
    def getJobsWithOutputFiles(self, projectId=None, fileTypeId=None, projectName=None, fileType=None, fileTypeClass=None,
        ...
        selcom, args, allowAny = self.refineSelectionCommand(
            selcom1, args1, fileTypeId, subType, contentFlag)

        selcom += ' AND ImportFiles.ImportId IS NULL'
        if topLevelOnly:
            selcom = selcom + ' AND Jobs.ParentJobId IS NULL'
        selcom += ' ORDER BY Jobs.CreationTime DESC'
        # print 'CDbApi.getJobsWithOutputFiles',selcom,args
        self.execute(selcom, args)
        jobList = self.fetchAll2PyList(
            [str, str, UUIDTYPE, str, str, int, int, str, float])
        ...
        if importFiles:
            ...
            selcom, args, allowAny = self.refineSelectionCommand(
                selcom, args, fileTypeId, subType, contentFlag)
        ...
        if subType is not None and allowAny:
            selcom2, args2, allowAny = self.refineSelectionCommand(
                selcom1, args1[0:1], fileTypeId, None, contentFlag)
            selcom2 += ' AND ImportFiles.ImportId IS NULL'
            if topLevelOnly:
                selcom2 = selcom2 + ' AND Jobs.ParentJobId IS NULL'
            ...
            jobList.extend(self.fetchAll2PyList(
                [str, str, UUIDTYPE, str, str, int, int, str, float]))
    ...
    def updateFile(self, fileId=None, key=None, value=None):
        ...
        if key is None:
            raise CException(self.__class__, 230, 'None')
        key = key.lower()
        ...
        # self.setDiagnostic(True)
        self.execute("UPDATE Files SET "+key +
                     " = ? WHERE FileID = ?", (value, fileId,))
        # self.setDiagnostic(False)
        self.commit()
        self.fileUpdated.emit({'fileId': fileId, 'jobId': jobId,
                              'projectId': projectId, 'key': key, 'value': value})
    ...
    def createComment(self, jobId=None, projectId=None, userName=None, comment=None):
        ...
        if jobId is None and projectId is None:
            raise CException(self.__class__, 240, 'None')
        ...
        if jobId is not None:
            ...
            self.execute(
                "INSERT INTO Comments (CommentId,JobID,UserID,TimeOfComment,Comment) VALUES (?,?,?,?,?);", args)
            self.commit()
            self.commentEdited.emit(
                {'jobId': jobId, 'commentId': commentId, 'comment': comment})
        else:
            commentId = self.uniqueId(
                table='ProjectComments', identifier='ProjectCommentID')
            args = [commentId, projectId, userId, time, comment]
            self.execute(
                "INSERT INTO ProjectComments (projectCommentId,projectID,UserID,TimeOfComment,Comment) VALUES (?,?,?,?,?);", args)
            self.commit()
            self.projectCommentEdited.emit(
                {'projectId': projectId, 'commentId': commentId, 'comment': comment})
    ...
    def updateComment(self, commentId, comment=None):
        ...
        time = self.currentTime()
        self.execute(
            'SELECT JobId FROM Comments WHERE CommentId = ?', (commentId,))
        jobIdList = self.fetchAll2Py(UUIDTYPE)
        if len(jobIdList) == 1:
            self.execute(
                "UPDATE Comments SET Comment = ?, TimeOfComment = ? WHERE CommentID = ?", (comment, time, commentId))
            self.commit()
            self.commentEdited.emit(
                {'jobId': jobIdList[0], 'commentId': commentId, 'comment': comment})
        else:
            self.execute(
                'SELECT ProjectId FROM ProjectComments WHERE ProjectCommentId = ?', (commentId,))
            projectIdList = self.fetchAll2Py(UUIDTYPE)
            if len(projectIdList) == 1:
                self.execute(
                    "UPDATE ProjectComments SET Comment = ?, TimeOfComment = ? WHERE ProjectCommentID = ?", (comment, time, commentId))
                self.commit()
                self.projectCommentEdited.emit(
                    {'projectId': projectIdList[0], 'commentId': commentId, 'comment': comment})
    ...
    def deleteComment(self, commentId):
        ...
        if len(jobIdList) == 1:
            ...
            self.commit()
            self.commentDeleted.emit(
                {'jobId': jobIdList[0], 'commentId': commentId})
        else:
            self.execute(
                'SELECT ProjectId FROM ProjectComments WHERE ProjectCommentId = ?', (commentId))
            projectIdList = self.fetchAll2Py(UUIDTYPE)
            ...
            if len(projectIdList) == 1:
                self.execute(
                    "DELETE FROM ProjectComments WHERE ProjectCommentID = ?", (commentId,))
                self.commit()
                self.projectCommentDeleted.emit(
                    {'projectId': projectIdList[0], 'commentId': commentId})
    ...
    def getProjectTags(self, projectId=None, tagText=False):
        if tagText:
            ...
            self.execute(
                'SELECT TagId FROM ProjectTags WHERE ProjectId = ?', (projectId,))
    ...
    def createProjectTag(self, projectId=None, tagId=None, commit=True):
        if self.projectTagExists(projectId, tagId):
            ...
            self.execute(
                "INSERT INTO ProjectTags (ProjectId,TagId) VALUES (?,?)", (projectId, tagId))
            if commit:
                self.commit()
    ...
    def projectTagExists(self, projectId=None, tagId=None):
        self.execute(
            'SELECT ProjectId,TagId FROM ProjectTags WHERE ProjectId = ? AND TagId = ?', (projectId, tagId))
    ...
    def deleteProjectTag(self, projectId=None, tagId=None):
        self.execute(
            "DELETE FROM ProjectTags WHERE ProjectID = ? AND TagId = ?", (projectId, tagId))
        self.commit()
        self.projectTagsChanged.emit(projectId)
    ...
    def createTag(self, text, parentTagId=None):
        ...
        self.execute("INSERT INTO Tags (TagId,text,parentTagId) VALUES (?,?,?)", [
                     tagId, text, parentTagId])
        self.commit()
        self.tagCreated.emit(
            {'tagId': tagId, 'parentTagId': parentTagId, 'text': text})
        return tagId
    ...
    def updateTag(self, tagId, key=None, value=None):
        ...
        if key == 'text':
            ...
            if len(ret) > 0:
                raise CException(self.__class__, 294, ret)
        self.execute("UPDATE Tags SET "+key +
                     " = ? WHERE TagID = ?", (value, tagId,))
        self.commit()
    ...
    def deleteTag(self, tagId, deleteChildren=False):
        ...
        if parentId is not None:
            if deleteChildren:
                ...
                self.execute(
                    "UPDATE Tags SET ParentTagId = NULL  WHERE ParentTagID = ?", (parentId))
    ...
    def deleteUnusedTags(self):
        self.execute(
            'DELETE FROM Tags WHERE Tags.TagId NOT IN (SELECT DISTINCT TagId FROM ProjectTags)', ())
        self.commit()
    ...
    def getTagList(self):
        ...
        self.execute(
            'SELECT Tags.TagId, Tags.ParentTagId, Tags.Text FROM Tags ORDER BY Tags.Text')
    ...
    def setDirectoryAlias(self, alias=None, directory=None):
        ...
        if not self._userRole in [USER_ROLE_OWNER]:
            raise CException(self.__class__, 106, 'You are ' +
                             self._userName+' calling createDirectoryAlias')
        ...
        if diry is None:
            ...
            self.execute(
                "INSERT INTO DirectoryAliases (DirectoryAlias,Directory) VALUES (?,?);", args)
        else:
            args = (directory, alias)
            self.execute(
                "UPDATE DirectoryAliases SET Directory= ? WHERE DirectoryAlias= ?", args)
        self.commit()
    ...
    def getAliasDirectory(self, alias=None):
        ...
        args = (str(alias),)
        self.execute(
            'SELECT Directory FROM DirectoryAliases WHERE DirectoryAlias = ?', args)
        rv = self.fetchAll2Py(str)
        if len(rv) == 0:
    ...
    def getRecentlyFinishedJobs(self, after=0.0):
        ...
        args = (str(after),)
        self.execute(
            'SELECT JobId,ProjectId,Status,JobTitle,FinishTime,ParentJobId FROM Jobs WHERE finishtime> ? ORDER BY finishtime', args)
        finishedJobs = self.fetchAll2PyList(
            (UUIDTYPE, UUIDTYPE, int, str, float, UUIDTYPE))
        ...
        for jobId, projectId, status, jobTitle, finishTime, parentJobId in finishedJobs:
            ...
            if status == JOB_STATUS_TO_DELETE:
                self.jobToDelete.emit({'jobId': jobId, 'projectId': projectId, 'status': status,
                                      'jobTitle': jobTitle, 'finishTime': finishTime, 'parentJobId': parentJobId})
            else:
    ...
    def getRecentlyStartedJobs(self, after=0.0):
        ...
        self.execute(
            'SELECT JobId,ProjectId,Status,ParentJobId,TaskName FROM Jobs WHERE creationtime> ? AND STATUS= ? ORDER BY creationtime', args)
        startedJobs = self.fetchAll2PyList(
            (UUIDTYPE, UUIDTYPE, int, UUIDTYPE, str))
        # print 'getRecentlyStartedJobs jobList',startedJobs

        for jobId, projectId, status, parentJobId, taskName in startedJobs:
            # print 'CDbApi.getRecentlyFinishedJobs',jobId
            self.jobStarted.emit({'jobId': jobId, 'projectId': projectId,
                                 'status': status, 'parentJobId': parentJobId, 'taskName': taskName})
    ...
    def getRunningSubJob(self, jobId):
        ...
        # in that context
        self.execute('SELECT JobId,TaskName FROM  Jobs WHERE parentJobId = ? AND STATUS = ?',
    ...
    def getRunningJobs(self, remote=False):
        if not remote:
            self.execute('SELECT JobId,JobNumber,TaskName,ProjectId,ProcessId,parentJobId FROM Jobs WHERE STATUS = ?', (str(
                JOB_STATUS_RUNNING)))
        else:
        ...
        ret = self.fetchAll2PyList(
            (UUIDTYPE, str, str, UUIDTYPE, int, UUIDTYPE))
        return ret
    ...
    def jobDirectory(self, jobId=None, jobNumber=None, projectId=None, projectDirectory=None):
        ...
        if jobNumber is None or (projectId is None and projectDirectory is None):
            jobInfo = self.getJobInfo(
                jobId=jobId, mode=['projectid', 'jobnumber'])
            jobNumber = jobInfo['jobnumber']
            if projectDirectory is None:
                projectDirectory = self.getProjectInfo(
                    projectId=jobInfo['projectid'], mode='projectdirectory')
        elif projectDirectory is None:
            projectDirectory = self.getProjectInfo(
                projectId=projectId, mode='projectdirectory')
    ...
    def createXData(self, jobId=None, dataClass=None, dataXml=None, projectId=None):
        ...
        self.execute(
            "INSERT INTO XData (XDataID,JobID,XDataClass,XDataXml) VALUES (?,?,?,?)", args)
    ...
    def getXData(self, dataClass=None, projectId=None, jobId=None):
        ...
        if jobId is None:
            if dataClass is None:
                args = (projectId,)
                self.execute(
                    'SELECT XData.XDataId FROM XData INNER JOIN Jobs ON XData.JobId=Jobs.JobId WHERE Jobs.ProjectId = ? ', args)
            else:
                args = (dataClass, projectId)
                self.execute(
                    'SELECT XData.XDataId FROM XData WHERE XDataClass = ? INNER JOIN Jobs ON XData.JobId=Jobs.JobId WHERE Jobs.ProjectId = ? ', args)
        else:
            if dataClass is None:
                args = (jobId,)
                self.execute(
                    'SELECT XDataId FROM XData WHERE JobId = ? ', args)
            else:
                args = (dataClass, jobId)
                self.execute(
                    'SELECT XDataId FROM XData WHERE XDataClass = ? AND JobId = ? ', args)
        rv = self.fetchAll2Py(UUIDTYPE)
        return rv
    ...
    def getXDataInfo(self, xDataId=None, mode='all'):
        ...
        if mode.lower() in ['xdataclass', 'class', 'xdataxml', 'xml']:
            ...
            self.execute(
                'SELECT XDataClass,XDataXml FROM XData WHERE XDataID = ?', args)
            rv = self.fetchAll2PyList([str, str])
        rv = self.fetchAll2Py(str)
        # self.setDiagnostic(False)
        if len(rv) > 0:
    ...
    def getXDataByJobContext(self, contextJobId=None, dataClass=None, projectId=None):
        ...
        self.execute(
            'SELECT XDataId FROM XData WHERE JobId = ? AND XDataClass = ?', args)
        rv = self.fetchAll2Py(UUIDTYPE)
        if len(rv) > 0:
            return rv
    ...
    def createJobKeyValue(self, jobId=None, keyTypeId=None, keyTypeName=None, value=None):
        ...
        if isinstance(value, str):
            ...
            self.execute(
                "INSERT INTO JobKeyCharValues (JobId,KeyTypeId,Value) VALUES (?,?,?)", args)
        else:
            args = (jobId, keyTypeId, float(value))
            # print 'createJobKeyValues args',args
            self.execute(
                "INSERT INTO JobKeyValues (JobId,KeyTypeId,Value) VALUES (?,?,?)", args)
        self.commit()
    ...
    def updateJobKeyValue(self, jobId=None, keyTypeId=None, keyTypeName=None, value=None):
        ...
        if isinstance(value, str):
            self.db.execute(
                "UPDATE JobKeyCharValues SET Value = ? WHERE JobID = ? AND KeyTypeId = ?", (value, jobId, keyTypeId))
        else:
            self.db.execute("UPDATE JobKeyValues SET Value = ? WHERE JobID = ? AND KeyTypeId = ?", (float(
                value), jobId, keyTypeId))
        self.commit()
    ...
    def deleteJobKeyValue(self, jobId=None, keyTypeId=None, keyTypeName=None):
        if keyTypeId is None and keyTypeName is not None:
            keyTypeId = self.getKeyTypeId(keyTypeName)
        self.db.execute(
            'DELETE FROM JobKeyValues WHERE JobId = ? AND KeyTypeId = ?', (jobId, keyTypeId,))
        self.db.execute(
            'DELETE FROM JobKeyCharValues WHERE JobId = ? AND KeyTypeId = ?', (jobId, keyTypeId,))

    def getFileAssociationTypeId(self, fileAssociationTypeName):
        u = (fileAssociationTypeName,)
        self.execute(
            'SELECT FileAssociationTypeID FROM FileAssociationTypes WHERE FileAssociationTypeName = ?', u)
        rv = self.fetchAll2Py(int)
        if len(rv) == 0:
    ...
    def createFileAssociation(self, typeId=None, typeName=None, associationName=None, fileList=[]):
        ...
        pid = self.uniqueId(table='FileAssociations',
                            identifier='FileAssociationID')

        args = [pid, typeId, associationName]
        self.execute(
            "INSERT INTO FileAssociations (FileAssociationID,FileAssociationTypeID,FileAssociationName) VALUES (?,?,?)", args)

        if len(fileList) > 0:
            for fileItem in fileList:
                if not isinstance(fileItem, (list, tuple)):
                    fileItem = [fileItem, 0]
                self.createFileAssociationMember(
                    self, fileAssociationId=pid, fileId=fileItem[0], roleId=fileItem[1], commit=False)
        self.commit()
        return pid

    def deleteFileAssociation(self, fileAssociationId):
        self.db.execute(
            'DELETE FROM FileAssociations WHERE FileAssociationId = ?', (fileAssociationId,))
        self.db.execute(
            'DELETE FROM FileAssociationMembers WHERE FileAssociationId = ?', (fileAssociationId,))
        self.commit()
    ...
    def removeFileAssociationMember(self, fileAssociationId=None, fileId=None, commit=True):
        self.db.execute(
            'DELETE FROM FileAssociationMembers WHERE FileId = ? AND FileAssociationId = ?', (fileId, fileAssociationId))
        if commit:
            self.commit()
        ...
        for assocId, assocTypeId, assocName, fileId, roleId in rv:
            if assocId not in info:
                info[assocId] = {'associationtypeid': assocTypeId,
                                 'associationname': assocName, 'members': []}
            info[assocId]['members'].append(
                {'fileId': fileId, 'roleId': roleId})

        return info

    def createProjectExport(self, projectId=None, projectExportAfter=None, projectExportSelection=None):
        pid = self.uniqueId(table='ProjectExports',
                            identifier='ProjectExportID')
        ...
        self.execute(
            "INSERT INTO ProjectExports (ProjectExportID,ProjectID,ProjectExportTime,ProjectExportAfter) VALUES (?,?,?,?)", args)
        self.commit()
        return pid
    ...
    def getProjectExportInfo(self, projectExportId=None, projectId=None, returnList=False):
        if projectExportId is not None:
            self.execute(
                'SELECT ProjectExportID,ProjectExportTime,ProjectExportAfter FROM ProjectExports WHERE ProjectExportId=?', (projectExportId,))
        elif projectId is not None:
            self.execute(
                'SELECT ProjectExportID,ProjectExportTime,ProjectExportAfter FROM ProjectExports WHERE ProjectId=? ORDER BY ProjectExportTime DESC', (projectId,))
    ...
    def getProjectImportInfo(self, projectImportId=None, projectId=None, returnList=False):
        ...
        if projectImportId is not None:
            self.execute(
                'SELECT ProjectImportID,ProjectImportTime,ProjectExportTime,ProjectExportDatabaseId FROM ProjectImports WHERE ProjectImportId=?', (projectImportId,))
        elif projectId is not None:
        ...
        if returnList or projectId is not None:
            ...
            for ii in range(len(rv)):
                if databaseHostNames.get(rv[ii][3], None) is None:
                    databaseHostNames[rv[ii][3]] = self.getDatabaseInfo(databaseId=rv[ii][3])[
                        'creationhostname']
            ...
            for item in ('projectimportid', 'projectimporttime', 'projectexporttime'):
                ret[item.lower()] = rv[0][ii]
                ii += 1
            ret['projectexportdatabase'] = self.getDatabaseInfo(databaseId=rv[0][ii])[
                'creationhostname']
            return ret
    ...
    def createDatabase(self, databaseId=None, creationTime=None, creationHostName=None,
        ...
        loaded = self.idExists(
            idValue=databaseId, table='Databases', idName='DatabaseID')
        if loaded:
            return True
        args = [databaseId, creationTime, creationHostName,
                createUserName, schemaVersion, schemaDate]
        self.execute(
            "INSERT INTO Databases (DatabaseId,CreationTime,CreationHostName,CreationUserName,SchemaVersion,SchemaDate) VALUES (?,?,?,?,?,?)", args)
        self.commit()
        return False
    ...
    def getDatabaseInfo(self, databaseId=None, returnList=False):
        if databaseId is None:
            ...
            self.execute(
                'SELECT DatabaseId,CreationTime,CreationHostName,CreationUserName,SchemaVersion,SchemaDate FROM Databases WHERE ThisIsMe=?', (1,))
        else:
            self.execute(
                'SELECT DatabaseId,CreationTime,CreationHostName,CreationUserName,SchemaVersion,SchemaDate FROM Databases WHERE DatabaseId=?', (databaseId,))
    ...
    def createProjectImport(self, projectId=None, projectExportId=None, projectExportDatabaseId=None, projectExportTime=None, projectExportAfter=None):
        ...
        if projectExportId is None or projectExportDatabaseId is None or not self.idExists(projectExportDatabaseId, 'Databases'):
            ...
            return
        pid = self.uniqueId(table='ProjectImports',
                            identifier='ProjectImportID')
        importTime = self.currentTime()

        args = [pid, projectId, importTime, projectExportId,
                projectExportTime, projectExportDatabaseId]
    ...
    def getJobTree(self, jobId=None, projectId=None, ifBack=True):
        ...
        self.execute(
            'SELECT JobID,JobNumber,TaskName,CreationTime,ProjectId,FileID,JobParamName,FiletypeID,ChildJobId FROM TempJobTree')
        ret = self.fetchAll2PyList(
            [str, str, str, float, str, str, str, int, str])
    ...
    def updateServerJob(self, jobId, name, value):
        self.execute('UPDATE ServerJobs SET '+name +
                     '= ? WHERE JobId = ?', (value, jobId))
        self.commit()
    ...
    def getServerJobs(self):
        ...
        self.execute(
            'SELECT JobId,Machine,Username,Mechanism,RemotePath,CustomCodeFile,Validate,KeyFilename,ServerProcessId,ServerGroup FROM ServerJobs')
        ret = self.fetchAll2PyList(
            [UUIDTYPE, str, str, str, str, str, str, str, int, str])
        return ret
...
class CDbXml(QtCore.QObject):
    ...
    @staticmethod
    def updateInstances():
        CDbXml.Instances = set(
            [obj for obj in CDbXml.Instances if isAlive(obj)])
    ...
    def createProject(self, projectDirectory=None):
        ...
        print('Attempting to create database project:', self.projectName,
              'with directory', self.projectDirectory, 'and projectId', self.projectId)
        try:
            ...
            pass
        try:
            pid = self.db.createProject(
                projectName=self.projectName, projectDirectory=self.projectDirectory, projectId=self.projectId)
        except CException as e:
            return e
        except:
    ...
    def loadJob(self, root, parentJobId=None):
        ...
        try:
            ...
            self.errReport.append(
                self.__class__, 5, 'Project: '+str(self.projectName))
            return False
        creationTime = root.findtext('creationtime')
        if creationTime is not None:
            creationTime = time.mktime(
                time.strptime(creationTime, CDbApi.TIMEFORMAT))
        finishTime = root.findtext('finishtime')
        if finishTime is not None:
            finishTime = time.mktime(
                time.strptime(finishTime, CDbApi.TIMEFORMAT))
        try:
        ...
        try:
            ...
            testJobId = self.db.getJobId(
                projectId=self.projectId, jobNumber=jobNumber)
            self.errReport.append(self.__class__, 4, 'Project: '+str(
                self.projectName)+' JobNumber: '+str(jobNumber), stack=False)
            return False
        except:
            pass

        args = [jobId, self.projectId, parentJobId, jobNumber, userAgent,
                taskName, jobTitle, creationTime, finishTime, statusId, evaluation]
        if self._diagnostic:
            print('CDbXml.createJob', args)
        self.db.execute(
            "INSERT INTO Jobs (JobID,ProjectID,parentJobId,JobNumber,userAgent,TaskName,JobTitle,CreationTime,FinishTime,Status,Evaluation) VALUES (?,?,?,?,?,?,?,?,?,?,?)", args)
    ...
    '''
        ...
        if impRoot is None:
            ...
            importId = self.db.createImportFile(
                sourceFileName=sourceFilename, annotation=annotation, jobId=jobId)
        ...
        args = [fileId, jobId, importId, baseName,
                relPath, fileTypeId, annotation,]
        if self._diagnostic:
            print('CDbXml.createFile args', args)
        self.db.execute(
            "INSERT INTO Files (FileID,JobID,ImportId,Filename,RelPath,FiletypeID,Annotation) VALUES (?,?,?,?,?,?,?)", args)
    ...
    def makeAllSql(self):
        ...
        self.makeSql('FileAssociationMembers',
                     self.db.FILEASSOCIATIONMEMBERITEMS)
        ...
        self.makeSql('TempFileAssociationMembers',
                     self.db.FILEASSOCIATIONMEMBERITEMS)
    ...
    def loadTable(self, commitPolicy=None, newJobNumber=None, selectJobIdList=None, updateJobStatus=False):
                ...
                if selectJobIdList is None or jobId in selectJobIdList:
                    if self.db.idExists(jobId, 'Jobs'):
                        if not updateJobStatus:
                            self.errReport.append(
                                self.__class__, 10, 'JobId '+str(jobId))
                        else:
                            # Assume this is from a remotely run job - update status
                            for key in ['status', 'finishtime']:
                                value = rowData[self.db.JOBITEMS.index(key)]
                                self.db.execute(
                                    "UPDATE Jobs SET "+key+" = ? WHERE JobID = ?", (value, jobId,))
                            loadedJob = (
                                rowData[self.db.JOBITEMS.index('jobnumber')], None)
                    else:
                        ...
                        try:
                            ...
                            oldJobNumber = rowData[self.db.JOBITEMS.index(
                                'jobnumber')]
                            parentJobId = rowData[self.db.JOBITEMS.index(
                                'parentjobid')]
                            if newJobNumber is None:
                                ...
                                rowData[self.db.JOBITEMS.index(
                                    'jobnumber')] = str(newJobNumber)
                                self.errReport.append(self.__class__, 9, 'Imported number ' + str(
                                    oldJobNumber) + ' changed to ' + str(newJobNumber))
                                self.loadedJobs[jobId] = (
                                    oldJobNumber, newJobNumber)
                                newJobNumber += 1
                            else:
                                if parentJobId in self.loadedJobs:
                                    subJobNumber = str(
                                        self.loadedJobs[parentJobId][1]) + rowData[self.db.JOBITEMS.index('jobnumber')].split('.')[-1]
                                    rowData[self.db.JOBITEMS.index(
                            ...
                            self.errReport.append(
                                self.__class__, 123, str(jobId))
                        if self._diagnostic:
                            print('CDbXml create Job', rowData)
                        try:
                            self.db.execute(self.sql['Jobs'], rowData)
                        except Exception as e:
                            self.errReport.append(
                                self.__class__, 210, str(rowData)+' '+str(e))
                        else:
                            if loadedJob is not None:
                                self.loadedJobs[jobId] = loadedJob
        ...
        if len(self.loadedJobs) > 0:
            ...
            self.db.execute('UPDATE Projects SET LastJobNumber=? WHERE ProjectId = ?', (int(
                lastJobNumber), self.projectId))
        ...
        try:
            ...
            for rowTree in table.getchildren():
                ...
                if (rowData[idx] not in self.loadedJobs) and (not self.db.idExists(rowData[idx], 'Jobs', 'JobId')):
                    ...
                    if self._diagnostic:
                        print('CDbXml create File', rowData)
                    try:
                        self.db.execute(self.sql['Files'], rowData)
                        loadedFiles.append(rowData[0])
                    except:
                        self.errReport.append(
                            self.__class__, 211, self.sql['Files']+' '+str(rowData))
            ...
            for rowTree in table.getchildren():
                ...
                if (not rowData[idx] in loadedFiles) and (not self.db.idExists(rowData[idx], 'Files', 'FileId')):
                    ...
                    try:
                        ...
                        self.errReport.append(
                            self.__class__, 212, str(rowData)+' '+str(e))
        ...
        try:
            ...
            for rowTree in table.getchildren():
                ...
                if (not not rowData[idx] in loadedFiles) and (not self.db.idExists(rowData[idx], 'Files', 'FileId')):
                    ...
                    if self._diagnostic:
                        print('CDbXml create ExportFile', rowData)
            ...
            for rowTree in table.getchildren():
                ...
                if (rowData[idx] not in self.loadedJobs) and (not self.db.idExists(rowData[idx], 'Jobs', 'JobId')):
                    ...
                    try:
                        ...
                        if self._diagnostic:
                            print('CDbXml create XData', rowData)
                        try:
                            self.db.execute(self.sql['XData'], rowData)
                        except:
                            self.errReport.append(
                                self.__class__, 213, 'XData '+str(rowData))
            ...
            for rowTree in table.getchildren():
                ...
                if self.db.idExists(rowData[self.db.COMMENTITEMS.index('commentid')], 'Comments'):
                    ...
                    if (rowData[idx] not in self.loadedJobs) and (not self.db.idExists(rowData[idx], 'Jobs', 'JobId')):
                        ...
                        self.errReport.append(
                            self.__class__, 215, 'Comment '+str(rowData))
                    else:
                        if not self.db.idExists(rowData[self.db.COMMENTITEMS.index('userid')], 'Users'):
                            # user not recognised in this database !!!!
                            rowData[self.db.COMMENTITEMS.index(
                                'userid')] = None
                            self.errReport.append(
                                self.__class__, 214, 'Comment '+str(rowData))
                        if self._diagnostic:
                            print('CDbXml.create comment', rowData)
                        try:
                            self.db.execute(self.sql['Comments'], rowData)
                        except:
                            self.errReport.append(
                                self.__class__, 213, 'Comment '+str(rowData))
            ...
            for rowTree in table.getchildren():
                ...
                if self.db.idExists(rowData[self.db.PROJECTCOMMENTITEMS.index('projectcommentid')], 'ProjectComments'):
                    ...
                    if (not self.projectId == rowData[idx]) and (not self.db.idExists(rowData[idx], 'Projects', 'ProjectId')):
                        # job not recognised in this database !!!!
                        self.errReport.append(
                            self.__class__, 215, 'ProjectComment '+str(rowData))
                    else:
                        if not self.db.idExists(rowData[self.db.PROJECTCOMMENTITEMS.index('userid')], 'Users'):
                            # user not recognised in this database !!!!
                            rowData[self.db.PROJECTCOMMENTITEMS.index(
                                'userid')] = None
                            self.errReport.append(
                                self.__class__, 214, 'ProjectComment '+str(rowData))
                        if self._diagnostic:
                            print('CDbXml.create projectcomment', rowData)
                        try:
                            self.db.execute(
                                self.sql['ProjectComments'], rowData)
                        except:
                            self.errReport.append(
                                self.__class__, 213, 'ProjectComment '+str(rowData))
                ...
                for rowTree in table.getchildren():
                    ...
                    if 0:   # Need test if item already in db
                        ...
                        if (rowData[idx] not in self.loadedJobs) and (not self.db.idExists(rowData[idx], 'Jobs', 'JobId')):
                            # job not recognised in this database !!!!
                            self.errReport.append(
                                self.__class__, 215, label+' '+str(rowData))
                        else:
                            try:
            ...
            for rowTree in table.getchildren():
                ...
                if self.db.idExists(rowData[self.db.FILEASSOCIATIONITEMS.index('fileassociationid')], 'FileAssociations'):
                    ...
                    try:
                        ...
                        self.errReport.append(
                            self.__class__, 213, 'FileAssociations '+str(rowData))
        ...
        try:
            ...
            for rowTree in table.getchildren():
                ...
                if 0:  # need to test if alrady loaded
                    ...
                    try:
                        self.db.execute(
                            self.sql['FileAssociationMembers'], rowData)
                    except:
                        self.errReport.append(
                            self.__class__, 213, 'FileAssociationMembers '+str(rowData))
    ...
    def createTempTables(self, fileName=None):
        ...
        if fileName is None:
            from core import CCP4Utils
            fileName = os.path.join(
                CCP4Utils.getCCP4I2Dir(), 'dbapi', 'temp_database_schema.sql')
                ...
                for rowTree in table.getchildren():
                    ...
                    if self._diagnostic:
                        print('CDbXml create JobKeyValue',
                              rowData, self.sql[tempTableName])
                    try:
                        self.db.execute(self.sql[tempTableName], rowData)
                    except:
                        self.errReport.append(
                            self.__class__, 213, tableName+' '+str(rowData))
        ...
        try:
            ...
            for rowTree in table.getchildren():
                ...
                if self.db.idExists(rowData[self.db.PROJECTCOMMENTITEMS.index('projectcommentid')], 'ProjectComments'):
                    ...
                    try:
                        self.db.execute(
                            self.sql['TempProjectComments'], rowData)
                    except:
                        self.errReport.append(
                            self.__class__, 213, 'ProjectComment '+str(rowData))
        ...
        try:
            ...
            for rowTree in table.getchildren():
                rowData = self.getRowFromEtree(
                    rowTree, self.db.FILEASSOCIATIONITEMS, self.db.FILEASSOCIATIONTYPES)
                if self._diagnostic:
                    print('CDbXml create FileAssociations')
                try:
                    self.db.execute(self.sql['TempFileAssociations'], rowData)
                except:
                    self.errReport.append(
                        self.__class__, 213, 'FileAssociation '+str(rowData))
        ...
        try:
            ...
            for rowTree in table.getchildren():
                rowData = self.getRowFromEtree(
                    rowTree, self.db.FILEASSOCIATIONMEMBERITEMS, self.db.FILEASSOCIATIONMEMBERTYPES)
                if self._diagnostic:
                    print('CDbXml create FileAssociationMembers')
                try:
                    self.db.execute(
                        self.sql['TempFileAssociationMembers'], rowData)
                except:
                    self.errReport.append(
                        self.__class__, 213, 'FileAssociationMember '+str(rowData))
            ...
            for rowTree in table.getchildren():
                ...
                if self.db.projectTagExists(rowData[self.db.PROJECTTAGITEMS.index('projectid')], rowData[self.db.PROJECTTAGITEMS.index('tagid')]):
                    ...
                    try:
                        ...
                        self.errReport.append(
                            self.__class__, 213, 'ProjectTags '+str(rowData))
    ...
    def setExclInTempTables(self):
        ...
        if self._diagnostic:
            self.db.execute('SELECT JobId FROM Jobs WHERE ProjectId=? AND Status IN (?,?,?,?)', [
                            self.projectId, JOB_STATUS_FINISHED, JOB_STATUS_FAILED, JOB_STATUS_UNSATISFACTORY, JOB_STATUS_FILE_HOLDER])
            jobIdList = self.db.fetchAll2Py(str)
            print('setExclInTempTables TempJobs finished/failed excl=2', jobIdList)
            self.db.execute('SELECT JobId FROM Jobs WHERE ProjectId=? AND Status NOT IN (?,?,?,?)', [
                            self.projectId, JOB_STATUS_FINISHED, JOB_STATUS_FAILED, JOB_STATUS_UNSATISFACTORY, JOB_STATUS_FILE_HOLDER])
            jobIdList = self.db.fetchAll2Py(str)
            print('setExclInTempTables TempJobs not finished/failed excl=1', jobIdList)

        self.db.execute('UPDATE TempJobs SET Excl=2 WHERE JobId IN (SELECT JobId FROM Jobs WHERE ProjectId=? AND Status IN (?,?,?,?))', [
                        self.projectId, JOB_STATUS_FINISHED, JOB_STATUS_FAILED, JOB_STATUS_UNSATISFACTORY, JOB_STATUS_FILE_HOLDER])

        self.db.execute('UPDATE TempJobs SET Excl=1 WHERE JobId IN (SELECT JobId FROM Jobs WHERE ProjectId=? AND Status NOT IN (?,?,?,?))', [
                        self.projectId, JOB_STATUS_FINISHED, JOB_STATUS_FAILED, JOB_STATUS_UNSATISFACTORY, JOB_STATUS_FILE_HOLDER])
        ...
        self.db.execute(
            'SELECT COUNT(*) FROM TempJobs WHERE Excl=0 and parentJobId IS NULL')
        nImportJobs = self.db.fetchAll2Py(int)[0]
        if self._diagnostic:
            print('setExclInTempTables nImportJobs', nImportJobs)
        if nImportJobs > 0:
            self.db.execute(
                'SELECT MIN(CAST(JobNumber AS integer)) FROM TempJobs WHERE Excl=0 and parentJobId IS NULL')
            importMin = self.db.fetchAll2Py(int)[0]
            self.db.execute(
                'SELECT MAX(CAST(JobNumber AS integer)) FROM TempJobs WHERE Excl=0 and parentJobId IS NULL')
            importMax = self.db.fetchAll2Py(int)[0]

            self.db.execute(
                'SELECT COUNT(*) FROM Jobs WHERE parentJobId IS NULL AND ProjectId = ?', (self.projectId,))
            nCurrentJobs = self.db.fetchAll2Py(int)[0]
            if nCurrentJobs > 0:
                self.db.execute(
                    'SELECT MAX(CAST(JobNumber AS integer)) FROM Jobs WHERE parentJobId IS NULL AND ProjectId = ?', (self.projectId,))
            ...
            if importMin <= currentMax:
                ...
                self.db.execute(
                    'UPDATE TempJobs SET NewJobNumber=updateJobNumber(JobNumber) WHERE Excl<=0')
        ...
        if self._diagnostic:
            ...
            self.db.execute(
                'SELECT FileId,FileName FROM TempFiles  WHERE JobId IN (SELECT JobId FROM TempJobs WHERE Excl = 2)')
            fileList = self.db.fetchAll2PyList([str, str])
            print(
                'setExclInTempTables TempFiles from Jobs not finished/failed excl=2', fileList)
            self.db.execute(
                'SELECT FileId,FileName FROM TempFiles WHERE FileId IN (SELECT FileId FROM Files)')
            fileList = self.db.fetchAll2PyList([str, str])
            print(
                'setExclInTempTables TempFiles fileId already in Files excl=1', fileList)
        self.db.execute(
            'UPDATE TempFiles SET Excl=1  WHERE JobId IN (SELECT JobId FROM TempJobs WHERE Excl = 2) OR FileId IN (SELECT FileId FROM Files)')

    def setExclImportedFiles(self):
        # mark the imported files as 'NOT excluded'
        if self._diagnostic:
            self.db.execute(
                'SELECT FileId,FileName FROM TempFiles WHERE Excl=0 AND FileId IN (SELECT FileId FROM TempImportFiles)')
            fileList = self.db.fetchAll2PyList([str, str])
            # print 'setExclImportedFiles imported files',fileList
        self.db.execute(
            'UPDATE TempFiles SET Excl=-1  WHERE Excl=0 AND FileId IN (SELECT FileId FROM TempImportFiles)')
    ...
    def cleanupTempTables(self):
        ...
        self.db.execute(
            'UPDATE TempFileUses SET Excl=1  WHERE JobId IN (SELECT JobId FROM TempJobs WHERE Excl > 0)')
    ...
    def importStats(self):
        ...
        stats['failedFiles'] = self.db.fetchAll2PyList(
            [str, str, str, int, str])
    ...
    def importProjectCommentsTempTables(self):
        ...
        tableList = ['ProjectComments', self.db.PROJECTCOMMENTITEMS,
                     self.db.PROJECTCOMMENTTYPES, 0]

        name, columns, typeList, excl = tableList
        columnText = columns[0]
        for item in columns[1:]:
            columnText = columnText + ','+item
    ...
    def importTempTables(self, testExl=True, includeCode=-1, ignoreList=[]):
        ...
        tableList = [['Jobs', self.db.JOBITEMS, self.db.JOBTYPES, includeCode],
                     ...
                     ['ImportFiles', self.db.IMPORTFILEITEMS,
                      self.db.IMPORTFILETYPES, 0],
                     ['ExportFiles', self.db.EXPORTFILEITEMS,
                      self.db.EXPORTFILETYPES, 0],
                     ...
                     ['FileAssociations', self.db.FILEASSOCIATIONITEMS,
                      self.db.FILEASSOCIATIONTYPES, 0],
                     ['FileAssociationMembers', self.db.FILEASSOCIATIONMEMBERITEMS,
                      self.db.FILEASSOCIATIONMEMBERTYPES, 0]
                     ]
        for name, columns, typeList, excl in tableList:
            ...
            if name == 'Jobs' and self.stats['incrJobNumber'] > 0:
                ...
                selectCom = ' SELECT '
                selectCom = selectCom + \
                    re.sub('jobnumber', 'newjobnumber',
                           columnText) + ' FROM Temp'+name
            else:
                ...
                selectCom = ' SELECT ' + columnText+' FROM Temp'+name
            if testExl:
                selectCom = selectCom + ' WHERE Excl='+str(excl)
            if self._diagnostic:
                valueCom = 'VALUES('
        ...
        for item in self.db.JOBITEMS[1:]:
            com = com + ',' + item
        ...
        for row in ret:
            ...
            for column in ['jobnumber', 'finishtime', 'status', 'evaluation', 'jobtitle']:
                self.db.execute('UPDATE Jobs SET '+column+'=? WHERE JobId =?',
                                (row[self.db.JOBITEMS.index(column)], row[0]))
    ...
    def listTempJobs(self, label=''):
        # Dignostic
        self.db.execute(
            'SELECT JobId,JobNumber,TaskName,Excl,NewJobNumber FROM TempJobs')
    ...
    def listTempFiles(self, label=''):
        # Dignostic
        self.db.execute(
            'SELECT TempJobs.JobNumber,TempFiles.FileId,TempFiles.FileName,TempFiles.Excl FROM TempFiles INNER JOIN TempJobs ON TempFiles.JobId = TempJobs.JobId')
    ...
    def importThisFile(self, jobNumber, fileName=None):
        ...
        if fileName is None:
            self.db.execute(
                'SELECT JobId,Excl,NewJobNumber FROM TempJobs WHERE JobNumber = ?', (jobNumber,))
            ...
            if len(ret) == 0:
                ...
                self.db.execute(
                    'UPDATE TempJobs SET Excl=-1 WHERE JobNumber = ?', (jobNumber,))
                if self._diagnostic:
                    print('CDbXml.importThisFile TempJobs', jobNumber, -1)
                return True, ret[0][2]
            elif ret[0][1] == 1:
                # Job is in db but is unfinished
                self.db.execute(
                    'UPDATE TempJobs SET Excl=-2 WHERE JobNumber = ?', (jobNumber,))
                if self._diagnostic:
                    print('CDbXml.importThisFile TempJobs', jobNumber, -2)
                return True, ret[0][2]
            elif ret[0][1] == -1:
                return True, ret[0][2]
            else:
            ...
            if len(ret) == 0:
                ...
                self.db.execute(
                    'UPDATE TempFiles SET Excl=-1 WHERE FileId = ?', (ret[0][3],))
                if self._diagnostic:
                    self.db.execute(
                        'SELECT FileId,FileName,Excl FROM TempFiles WHERE FileId = ?', (ret[0][3],))
                    fileList = self.db.fetchAll2PyList([str, str, int])
                    if self._diagnostic:
                        print('CDbXml.importThisFile TempFiles', fileList)
                return True, ret[0][2]
            elif ret[0][4] == -1:
                return True, ret[0][2]
            else:
      ...
      '''
            ...
            if val is None or val == 'None':
                ...
                try:
                    ...
                    rv.append(None)
                    self.errReport.append(
                        self.__class__, 212, 'Data type:'+str(att)+' is: '+str(val))
        return rv
                    ...
                    if jobId > 0:
                        ...
                        if dataType in jobs[jobId]:
                            if ['INPUT_FILES', 'OUTPUT_FILES'].count(dataType):
                                jobs[jobId][dataType] = self.splitDefList(
                                    value)
...
def testModule():
    ...
    from core import CCP4Utils
    if os.path.exists(testQtDb.TESTDBFILE):
        os.remove(testQtDb.TESTDBFILE)
    if os.path.exists(testSqliteDb.TESTDBFILE):
        os.remove(testSqliteDb.TESTDBFILE)
...
class testDb(unittest.TestCase):
    ...
    def test0User(self):
        ...
        try:
            ...
            self.assertEqual(
                e[0]['code'], 120, 'Wrong error when attempting to create user with same name')
        except:
            self.fail(
                'Unknown error when attempting to create user with same name')
        else:
        ...
        try:
            ...
            self.assertEqual(
                e[0]['code'], 106, 'Wrong error when attempting to create user with wrong permission')
        except:
            self.fail(
                'Unknown error when attempting to create user with wrong permission')
        else:
            self.fail(
                'No error when attempting to create user with wrong permission')
        ...
        self.db = CDbApi(
            mode=self.mode, fileName=self.TESTDBFILE, userName='me')
        uList = self.db.listUsers(True)
        print('test0User uList', uList)
        self.assertEqual(uList[1][1], 'another',
                         'Failed changing user name and listing')

        info = self.db.getUserInfo(uid0)
        # print 'after getUserInfo',info
        self.assertEqual(info['username'], 'another',
                         'Error calling getUserInfo')

    def test1Project(self):

        pid0 = self.db.createProject(
            'myproject', projectDirectory=self.TESTDBDIR)
        pid1 = self.db.getProjectId('myproject')
        self.assertEqual(
            pid0, pid1, 'Error creating project and retrieving project ID')

        baby = self.db.createProject('babyproject', userName='me', parentProjectId=pid1,
                                     projectDirectory=os.path.join(self.TESTDBDIR, 'baby'))
        self.db.commit()
        self.db.close()
        ...
        self.assertEqual(pList[1][3], pid0,
                         'Failed creating child project and listing')

        self.db.updateProject(baby, key='projectDirectory',
                              value=os.path.join(self.TESTDBDIR, 'baby2'))
        pList = self.db.listProjects(True)
        self.assertEqual(os.path.split(
            pList[1][2])[-1], 'baby2', 'Failed updating project directory')
    ...
    def test2Aliases(self):
        ...
        self.assertTrue(('CCP4I2_TOP' in aliasList),
                        'Alias list does not have CCP4I2_TOP')
    ...
    def test3Jobs(self):
        ...
        fo1 = CCP4ModelData.CSeqDataFile(
            project='myproject', relPath='CCP4_JOBS/job_1', baseName='starting_data.seq')
        f1 = self.db.createFile(jobId=jid1, fileObject=fo1, sourceFileName=os.path.join(
            CCP4Utils.getTestTmpDir(), 'starting_data.seq'))

        jid2 = self.db.createJob(
            pid, 'import_xtal', status=JOB_STATUS_FINISHED)
        fo2 = CCP4XtalData.CMtzDataFile(
            project='myproject', relPath='CCP4_JOBS/job_2', baseName='starting_data.mtz')
        f2 = self.db.createFile(jobId=jid2, fileObject=fo2, sourceFileName=os.path.join(
            CCP4Utils.getTestTmpDir(), 'starting_data.mtz'))
        ...
        jid4 = self.db.createJob(
            pid, 'mr', parentJobId=jid3, jobTitle='testing a parentJobId and jobTitle')
        self.db.createFileUse(jobId=jid4, fileId=f1)
        self.db.createFileUse(jobId=jid4, fileId=f2)
        fo3 = CCP4ModelData.CPdbDataFile(
            project='myproject', relPath='CCP4_JOBS/job_3/job_1', baseName='model_1.pdb')
        fo4 = CCP4ModelData.CPdbDataFile(
            project='myproject', relPath='CCP4_JOBS/job_3/job_1', baseName='model_2.pdb')
        f3 = self.db.createFile(jobId=jid4, fileObject=fo3)
        f4 = self.db.createFile(jobId=jid4, fileObject=fo4)
        self.assertEqual(self.db.getJobInfo(jid4, 'ParentJobId'),
                         jid3, 'Error setting and retrieving parent job number')
        self.db.updateJobStatus(jid4, JOB_STATUS_FINISHED)

        # Two runs of model building
        jid5 = self.db.createJob(
            pid, 'build', parentJobId=jid3, jobTitle='build')
        self.db.createFileUse(jobId=jid5, fileId=f2)
        self.db.createFileUse(jobId=jid5, fileId=f3)
        fo5 = CCP4ModelData.CPdbDataFile(
            project='myproject', baseName='built.pdb', relPath='CCP4_JOBS/job_3/job_2')
        f5 = self.db.createFile(
            jobId=jid5, fileObject=fo5, fileTypeName='chemical/x-pdb')
        self.db.updateJobStatus(jid5, JOB_STATUS_FINISHED)

        fileIds = self.db.getJobFiles(
            jobId=jid5, role=FILE_ROLE_IN, mode='fileId')
        self.assertTrue(fileIds == [f2, f3] or fileIds == [
                        f3, f2], 'Failed restoring input file ids')
        fileNames = self.db.getJobFiles(
            jobId=jid5, role=FILE_ROLE_OUT, mode='fileName')
        self.assertEqual(fileNames, ['built.pdb'],
                         'Failed restoring input file ids')

        # Lets say second model building failed
        jid6 = self.db.createJob(
            pid, 'build', parentJobId=jid3, jobTitle='build')
        ...
        self.db.updateJobStatus(
            status='Finished', projectName='myproject', jobNumber='3')
        ...
        self.assertEqual(info['taskname'], 'auto_mr',
                         'Error retrieving taskName')
        fileTypeList = self.db.getJobFiles(
            jobId=jid3, role=FILE_ROLE_OUT, mode='fileType')
        # Output should be one PDB file
        # print 'test3Jobs fileTypeList',fileTypeList
        self.assertEqual(fileTypeList, ['chemical/x-pdb'])

        fileList = self.db.getJobFiles(
            jobId=jid5, role=FILE_ROLE_OUT, mode='fullPath')
        self.assertEqual(fileList, [os.path.join(
            self.TESTDBDIR, 'CCP4_JOBS', 'job_3', 'job_2', 'built.pdb')], 'Failed running getJobFiles')
        fileId, jobId = self.db.matchFileName(fileName=os.path.join(
            self.TESTDBDIR, 'CCP4_JOBS', 'job_3', 'job_2', 'built.pdb'))
    ...
    def test5glean(self):
        ...
        import os
        from core import CCP4Utils
        jid7 = self.db.createJob(
            projectId=pid, taskName='pdbset', jobTitle='test5glean')
        import shutil
        from core import CCP4Modules
        shutil.copyfile(os.path.join(CCP4Utils.getCCP4I2Dir(), 'test', 'data', 'test_dbapi.params.xml'),
                        CCP4Modules.PROJECTSMANAGER().makeFileName(jobId=jid7, mode='PARAMS'))
        ...
        fileList = self.db.getJobFiles(
            jobId=jid7, role=FILE_ROLE_IN, mode='fullPath')
        # print 'test5glean fileList',fileList
        self.assertEqual(len(
            fileList), 1, 'Error gleaning files from params - wrong number of input files')
        self.assertEqual(os.path.split(fileList[0])[
                         1], 'model_1.pdb', 'Error gleaning files from params - wrong file name')

        fileList = self.db.getJobFiles(jobId=jid7, role=FILE_ROLE_OUT)
        self.assertEqual(len(
            fileList), 1, 'Error gleaning files from params - wrong number of output files')
        filePath = self.db.getFullPath(fileId=fileList[0])
        self.assertEqual(os.path.split(filePath)[
                         1], 'XYZOUT.pdb', 'Error gleaning files from params - wrong output file name')

    def test6glean(self):
        pid = self.db.getProjectId('myproject')
        import os
        from core import CCP4Utils
        pf = os.path.join(CCP4Utils.getCCP4I2Dir(), 'test',
                          'data', 'test_dbapi_2.params.xml')
        import shutil
        from core import CCP4Modules
        jid8 = self.db.createJob(pid, 'newProject', jobTitle='test6glean')
        shutil.copyfile(os.path.join(CCP4Utils.getCCP4I2Dir(), 'test', 'data', 'test_dbapi_2.params.xml'),
                        CCP4Modules.PROJECTSMANAGER().makeFileName(jobId=jid8, mode='PARAMS'))
        ...
        containsSe = self.db.getXDataByJobContext(
            contextJobId=jid8, dataClass='CContainsSeMet', projectId=pid)
    ...
    def test7comment(self):
        ...
        self.assertEqual('Well whatever really',
                         commentList[0][3], 'Error creating and retrieving comment')
    '''
...
class testSqliteDb(testDb):
    ...
    TESTDBFILE = os.path.join(
        CCP4Utils.getTestTmpDir(), 'CCP4DbApi.testsqlite.db')

    def setUp(self):

        self.db = CDbApi(mode='sqlite', userName='me',
                         userPassword='foo', fileName=testSqliteDb.TESTDBFILE)
        self.mode = 'sqlite'
        from core import CCP4Modules
...
class testQtDb(testDb):
    ...
    def setUp(self):
        self.db = CDbApi(mode='qt_sqlite', fileName=testQtDb.TESTDBFILE,
                         userName='me', userPassword='foo')
        self.mode = 'qt_sqlite'
        from core import CCP4Modules
        CCP4Modules.PROJECTSMANAGER().setDatabase(self.db)
